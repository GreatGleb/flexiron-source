...
металл, которого там уже нет. `sale` по куску переводит его в `sold`, `return` — обратно в
`available`; остальные статусы куска ставит `PATCH /api/warehouse/offcuts/:id`.
