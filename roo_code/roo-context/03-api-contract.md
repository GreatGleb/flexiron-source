...
Предлагаются только СВОБОДНЫЕ куски: со статусом `available`, размер которых выражается
в единице партии, и которые ещё не названы в разбивке ни одной строки заказа. Обрезок —
один физический кусок, делить его нечем, поэтому занят он целиком с той минуты, как его
выбрали, — и задолго до резервирования. Тот же ответ даёт и сохранение: кусок, занятый
между показом списка и нажатием «Добавить», отклоняется кодом `OFFCUT_NOT_AVAILABLE`. **Обрезки не участвуют в автоматическом подборе по партиям (FIFO) и
