...
**Решено 2026-09-09 (П44): проверки не будет, потому что не будет удаления.** Товар помечается
**архивным** и остаётся в базе: он попадает в исторические документы, и заказ двухлетней давности
обязан читаться как был. Что это значит для эндпоинта:

- `DELETE /api/products/:id` перестаёт удалять строку и становится переводом в архив; `PRODUCT_IN_USE`
  не нужен вовсе — отказывать не в чем, а `PRODUCT_NOT_FOUND` остаётся;
- списочные эндпоинты по умолчанию архивные товары **не отдают**, и ни один выбор их не предлагает —
  ни строка заказа, ни запрос поставщику;
- `GET /api/products/:id` отдаёт архивный товар **наравне с живым**, с признаком архива: карточка,
  открытая по ссылке из старого заказа, обязана открыться и объяснить, почему товар не предлагается
  в новых.

Общее правило — [§22](00-conventions.md).

Семь чужих таблиц при этом перестают быть угрозой: висячих ссылок не возникает, потому что строка
не исчезает, и ни одна из политик FK не срабатывает. Ниже — как они устроены сегодня; **разных
политик три** (`RESTRICT`, `SET NULL`, `CASCADE`), а не пять, как говорил этот абзац до
2026-09-09 — считались варианты, а не политики:
...
`composables/useProductCard.ts:194`).

**Решено 2026-09-09 (П57): и это меняется — порог пересчитывает нехватку сразу.** `PATCH` товара с
изменённым `minStock` обязывает сервер пересчитать нехватку по этому товару в тот же момент:
остаток ниже нового порога — нехватка заводится и рождает `stock_deficit` по обычному правилу
перехода. Ждать складской операции нельзя, иначе порог подняли, товара не хватает, а не знает
никто. Симметрично: порог опустили или обнулили, остаток снова выше — нехватка, заведённая
порогом, тем же пересчётом закрывается (это чтение контракта, не слово владельца). Флаг тревоги
такой нехватке не полагается — он только у нехватки под подтверждённый заказ (П55,
[§10.4](00-conventions.md)). Механика эмиттеров — §10 соглашений.
...
при удалении товара — **решено 2026-09-09 (П44)**: ничего, потому что товар не удаляется, а
помечается архивным (см. `DELETE /api/products/:id` и [§22](00-conventions.md)).
...
5. **Проверка «товар используется» в моке — три захардкоженных id, а на схеме это три разные
   ...
   архивным, поэтому ни проверка, ни `PRODUCT_IN_USE` больше не нужны — чужие ссылки остаются
   действительными ([§22](00-conventions.md)).
...
| **снято 2026-09-10 (прецедент `settings`)** · алиасами на бэкенде: схема настроек уже отдаёт camelCase через `by_alias=True` (`crud/action.py:156`), и второго правила в проекте быть не должно — слой преобразования на клиенте пришлось бы держать в каждом сервисе | врезка «Регистр полей»; [§1](00-conventions.md) (БАГ-07, БАГ-11) |
...
| **снято 2026-09-10 (старшинство схемы)** · не остаётся: колонку удалила миграция `a1b2c3d4e5f6`, и отдавать поле, которого нет в таблице, сервер не может | графа 9, пункт 3 |
| **снято 2026-09-10 (работа, не решение)** · таблицей связи «товар ↔ поставщик» с ценой, единицей и сроком на самой связи; сегодня её нет ни одной — это незаконченный бэкенд, а не открытый вопрос | правило домена 11 |
| **снято 2026-09-10 (§13)** · остаётся, и он же прецедент для соседей: справочник отдаётся лёгким эндпоинтом, а не большим `pageSize`. Инвалидация ему не нужна — он читает то же хранилище, а не кэш | графа 9; [§13](00-conventions.md) |
