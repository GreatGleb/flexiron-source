...
Зовут список пятеро, и двумя разными способами: экран каталога — по монтированию
...
`sortBy: 'name'` (`views/admin/orders/AddOrderItemsModal.vue:415-418`,
...
- **его ждут вместе со своими данными, а не после.** Семь складских загрузчиков зовут
...
и `:17-22`, сборка — `.../get_product_detail/domain.py:81-104`.
...
2. **категория**: у сервера вложенный `category` из живой выборки (`.../get_product_detail/schemas.py:17-22`, сборка
   `.../get_product_detail/domain.py:56-64`), у фронта два плоских поля `categoryId` + `categoryName`
...
3. **`price_unit`** — легаси-подпись, которую сервер собирает заново из FK при каждом чтении
   (`.../get_product_detail/domain.py:26-44`, вызов `:77-79`), хотя колонку миграция удалила
...
4. **`updated_at`** сервер отдаёт (`.../get_product_detail/schemas.py:54`), фронт хранит только `createdAt`
...
6. **элемент `field_values`** у сервера — три поля, у фронта шесть, и `field_name` сервер заполняет
   **заглушкой** `str(fv.field_id)` с комментарием «placeholder — resolve field name»
   (`.../get_product_detail/domain.py:70`) — БАГ-04; фронтовый `ProductFieldValue` — `{fieldId, fieldName: TranslatedString,
...
(`.../get_product_detail/domain.py:51-53`) и отдаёт 404 с телом
`{"detail": {"message", "code"}}` (`.../get_product_detail/action.py:42-46`); мок
...
(`get_product_detail`) · схемы `.../get_product_detail/schemas.py:9-54` · выборка
`.../get_product_detail/repository.py:13-22` (**без фильтра по арендатору** — БАГ-14)
...
- **единицы**: незаданные каскадируются `warehouse ← sale`, `purchase ← warehouse`
  (`.../create_product/domain.py:38-41`).
...
`.../create_product/domain.py:63-67`, конверт и статус — `.../create_product/action.py:23`, `:38-42`.
...
(`.../create_product/domain.py:30-31`) и отдаёт 422 с телом `{"detail": {"message", "code"}}`
(`.../create_product/action.py:43-47`). Мок не бросает ничего
...
схемы `.../create_product/schemas.py:8-40` · запись `.../create_product/repository.py:48-50`
...
| единицы нового товара | `.../create_product/domain.py:38-41` (каскад `warehouse ← sale`, `purchase ← warehouse`) | сервер каскадирует, мок пишет каждую как пришла (`services/mocks/products.ts:14087-14089`) — БАГ-13 |
| `priceQuantity` | три экземпляра: `.../create_product/schemas.py:20`, `backend/app/modules/products/shared/models.py:132-134`, мок `services/mocks/products.ts:14084`, форма карточки `composables/useProductCard.ts:193` | значение одно (`1`), владельца нет |
...
2. **Правила пересчёта.** `settings.conversions` подставляют коэффициент в форму при загрузке и при
   ...
   не знает: у него каскадируются только сами единицы (`.../create_product/domain.py:38-41`).
...
(`.../get_product_detail/domain.py:47-53`, `:77-79`) — БАГ-14. Что модуль это умеет, видно рядом:
...
(`.../create_product/action.py:33-34`, `.../get_product_detail/action.py:34-35`). Во фронте
...
единственный `Depends` в обоих — `get_db` (`.../create_product/action.py:26`,
`.../get_product_detail/action.py:31`), тогда как соседний `GET /api/auth/me` требует Bearer
...
(`.../create_product/repository.py:48-50`), то есть 422 отката не требует — он и так до записи не
доходит (`.../create_product/domain.py:30-31`). **Save карточки шлёт один запрос на все три группы
...
2. **`avgSalePrice`** — нетто отгруженного, делённое на отгруженное количество (`:13891-13895`).
   ...
   (`.../get_product_detail/schemas.py:25-54`) — значит **сервер обязан считать их сам**, и это
...
3. **`price_unit`** — подпись вида `"EUR/kg"`, которую сервер собирает из `currency_id` +
   `sale_uom_id` при каждом чтении (`.../get_product_detail/domain.py:26-44`, вызов `:77-79`) после
   ...
   берётся `en → ru → lt` (`.../get_product_detail/domain.py:37-41`), то есть подпись всегда
...
4. **`categoryName`** — производное **только во фронте**: мок хранит его копией в записи товара
   ...
   сервер отдаёт вложенный `category` из живой выборки (`.../get_product_detail/domain.py:56-64`). У
...
2. **Товар идентифицируется UUID'ом на сервере и строкой `prod-NNN` в моке.** Сегмент пути
   типизирован `product_id: UUID` (`.../get_product_detail/action.py:30`), схема — `UUIDMixin`
...
10. **Дату создания выбирает сервер, и формат сегодня расходится.** Мок пишет
    ...
    (`.../get_product_detail/schemas.py:53-54`). Владелец времени — §14 соглашений.
...
15. **`weightPerWarehouseUnitKg` — величина, которой нет на бэкенде и нет в объявленной форме
    ...
    подпись `createProduct` его не принимает (`services/productsService.ts:29-46`) — БАГ-09.
16. **Коэффициенты пересчёта материализуются в товар из справочника настроек при первом же
    ...
    (`.../create_product/domain.py:38-41` каскадирует только единицы).
...
|---|---|
| `GET /api/products`: «Сортировка по `name ASC` (дефолт)», `sortBy` по умолчанию `"name"` (`03-api-contract.md:1006`, `03-api-contract.md:1021`) | мок без `sortBy` не сортирует вовсе — весь блок под `if (params.sortBy)` (`services/mocks/products.ts:13965-13974`), а клиент не шлёт параметр, пока пользователь не кликнул по колонке (`services/productsService.ts:17-20`, `composables/useProducts.ts:81-88`). Правило домена 7 |
| «`categoryIds: null` = без категории» (`03-api-contract.md:1021`) | товар с `categoryId: null` не попадёт ни в одну выборку: фильтр сравнивает `p.categoryId ?? ''` (`services/mocks/products.ts:13961`), а пустая строка в списке id не встречается. Правило домена 8 |
...
| `POST /api/products`: тело из шести полей (`03-api-contract.md:1030-1037`) | у сервера пятнадцать (`.../create_product/schemas.py:8-31`), у клиента те же пятнадцать в camelCase (`services/productsService.ts:29-46`); нет ни `priceQuantity`, ни `currencyId`, ни трёх UoM, ни четырёх полей пересчёта |
| «Response 200: `ApiResponse<Product>` — созданный товар целиком (с `fieldValues: []`, `linkedSuppliers: []`)» (`03-api-contract.md:1038`) | сервер отдаёт четыре поля — `{id, name, sku, message}` — и статус **201** (`.../create_product/schemas.py:34-40`, `.../create_product/action.py:23`) |
...
| `PATCH /api/products/:id`: дельта из шести ключей (`03-api-contract.md:1085-1091`) | объявленных шестнадцать (`services/productsService.ts:63-83`) плюс семнадцатый неявный — `weightPerWarehouseUnitKg` (БАГ-09) |
...
| `PATCH /api/products/:id` — шесть ключей из семнадцати, `fieldValues` полным массивом со справочными полями | раздел `PATCH /api/products/:id`; правило домена 9 |
...
| **осталось** · какое право нужно на чтение, создание, правку, удаление товара и на удаление записи его журнала | графа 7; решение владельца |
| **осталось** · кто владеет перечнем формул пересчёта и почему валюта представлена тремя способами | графа 1; решение владельца |
