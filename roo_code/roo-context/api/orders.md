...
применённой. Что обязано быть атомарным внутри одного Save и какова область действия ключа —
**строка владельца 6**.
...
1. **осталось** — кому принадлежат ставка НДС, скидка и валюта нового заказа, и кому — размер
   страницы списка. Маржа читается из настроек (`mocks/orders.ts:1633`), а остальные три стоят
   литералами `0`, `21`, `'EUR'` (`:1634`, `:1637`, `:1638`) при том, что настройки владеют всеми
   тремя (`types/settings.ts:15-18`).
2. **осталось** — обязан ли сервер сообщать об отгрузке, о выставленном счёте, о возврате товара и
...
4. **осталось** — чем ограничивается выборка заказа: ни таблицы, ни колонки, ни параметра
   арендатора у домена нет.
5. **осталось** — когда сервер начнёт вырезать себестоимость по праву `seeCost` и что фронт обязан
   делать в этом режиме. Сегодня право режет только записи истории с `sensitive: 'cost'`, а
   `unitCost`, `costSource`, `allocations` и `marginPercent` отдаются всем.
6. **осталось** — что обязано быть атомарным внутри одного Save карточки и какова область действия
   ключа идемпотентности. Одно нажатие Save рассыпается на `1 + N + M + K + L` последовательных
   запросов (`composables/useOrderCard.ts:418-533`), падение в середине оставляет первую половину
   применённой; ключ идемпотентности при этом лежит в `Map` на процесс, без срока и без привязки к
   пути (`mocks/index.ts:260-269`).
