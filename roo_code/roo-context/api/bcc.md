...
`selected` на каждой перезагрузке списка (`BccRequestPage.vue:513-520`).
...
(`BccRequestPage.vue:890`), а второго вызывающего у `getBccHistory` не существует
...
(`services/mocks/bcc.ts:258-266`) и отдаёт `slice` **исходного** массива, а не копию (`:261`);
порядок держится тем, что новая строка кладётся в начало (`:364`, `:386`) поверх сида, уложенного
...
Дальше массив правит только клиент: новые строки после отправки (`BccRequestPage.vue:292`) и
логирования (`:439`), после приёма ответа (`:359`) и отметки молчания (`:370`) — причём в обоих
первых случаях те же строки пишутся ещё и **прямо в мок-стор** (`:293`, `:440`), потому что мок
их не создаёт (БАГ-01, БАГ-11).
...
Ответ: `{ requestId: string }` и больше ничего (`services/bccService.ts:34`,
`services/mocks/bcc.ts:334`). **Строк истории ответ не несёт, и сервер их сегодня не создаёт**:
`mockSendBccRequest` пишет конверт в журнал и возвращает идентификатор
(`services/mocks/bcc.ts:310-335`), ленту не трогает — БАГ-11. Возвращённый `requestId` клиент
**выбрасывает** (`await send()` без присваивания, `BccRequestPage.vue:290`) и подставляет свой
номер, посчитанный по загруженной странице ленты как `max(req-NNN) + 1` (`:264-275`, вызов `:391`)
— БАГ-02. Формат этого поля на схеме — `String(50)`
(`backend/app/modules/bcc/shared/models.py:54-56`), то есть ограничения на вид номера нет.
...
(`:123-124`). Мок повторяет то же со своей стороны (`services/mocks/bcc.ts:319-332`), но **слабее
на два правила**: дубли не снимает и пустой список не отвергает (`:319-321`) — оба пути ошибки под
...
тему в текущей локали (`BccRequestPage.vue:280-284`), потом переносит `selectedRecipientIds` в
флаги `recipients[].selected` (`BccRequestPage.vue:286-288`), потому что композабл собирает
...
`:277-307` · мок `services/mocks/index.ts:910-916` → `services/mocks/bcc.ts:310-335`
...
(`BccRequestPage.vue:377`, дефолт `'Email'` `:379`, переводы `:381-387`). На схеме `source` —
...
Ответ: `{ requestId: string }` (`services/bccService.ts:56`). `mockLogBccRequest` — три строки: он
игнорирует весь payload (имя параметра `_payload`) и возвращает `req-${Date.now()}`
(`services/mocks/bcc.ts:337-343`). **Ни одной строки истории не создаётся** — БАГ-11; возвращённый
`requestId` клиент выбрасывает (`BccRequestPage.vue:430-437`) и считает свой (`:264-275`, вызов
`:391`) — БАГ-02.

**Отправка и логирование обязаны создавать строки одинаково, и сегодня это буквально одна
функция.** `createEventRows` (`BccRequestPage.vue:390-413`) вызывается обоими путями (`:291`,
`:438`) и отличается только значением `source`; она же кладёт `status: 'sent'` (`:408`) и в
логированной строке тоже. После успеха страница показывает тост и сбрасывает выбор товаров,
сохраняя преднастроенного получателя (`:441-450`).
...
`mockAcceptResponse` зовёт `notifySupplierResponse` безусловно (`services/mocks/bcc.ts:368` →
`services/mocks/notifications.ts:657-670`), поэтому «править ответ» у уже отвеченной строки (та же
`savePrice` из кнопки `BccRequestPage.vue:991`) рождает второе уведомление о том же ответе —
...
`unit` отсутствуют (`services/mocks/bcc.ts:372-388`, поля `:375-385`). Мок кладёт её в начало
ленты сам (`:386`), страница — ещё раз в свою копию (`BccRequestPage.vue:370`). На неизвестном
`eventId` — `null` в ленте (БАГ-04).

**Новая строка наследует `source` исходной** (`services/mocks/bcc.ts:383`), то есть «не ответил на
...
(`services/mocks/bcc.ts:365-367`). Отсутствие эмиттера здесь — решение, а не пропуск.
...
`notifySupplierResponse` вызывается из `mockAcceptResponse` (`services/mocks/bcc.ts:368` →
...
`no-response` — решение с записанной причиной (`services/mocks/bcc.ts:365-367`). **Расхождение с
...
события (`services/mocks/bcc.ts:368`), то есть попадает в чужое пространство идентификаторов —
...
— при том, что каждый создаёт новую строку (`services/mocks/bcc.ts:364`, `:386`) — БАГ-03.
...
конверт в журнале мока (`services/mocks/bcc.ts:323`, спека
`services/mocks/bcc-envelope.spec.ts:40-46`). **Атомарность записи не выражена нигде, и это
главное наблюдение графы:** «письмо ушло» и «в ленте появились N × M строк» — два разных места,
причём второе живёт **в браузере** (`BccRequestPage.vue:390-413`, запись `:293`, `:440`; мок строк
не создаёт — `services/mocks/bcc.ts:310-335`, `:337-343`) — БАГ-01, БАГ-11. Что входит в одну
транзакцию и что делать, если письмо ушло, а строки не записались — строка владельцу.
...
(4) `requestId` — **выводимое значение, которое сегодня считают оба и по-разному**: «сервер»
возвращает своё (`services/mocks/bcc.ts:334`, `:342`), клиент его выбрасывает и считает
`max(req-NNN) + 1` (`BccRequestPage.vue:264-275`) — БАГ-02. (5) `productCount` категории — на схеме
...
3. **Сервер сильнее мока на два правила отправки.** Дубли адресов снимаются с сохранением порядка
   ...
   Мок не делает ни того, ни другого (`services/mocks/bcc.ts:319-321`), а кода `NO_RECIPIENTS`
...
10. **Список получателей не сокращается никогда — совпадение по категории лишь всплывает наверх.**
    ...
    пряча остальных (`BccRequestPage.vue:204-213`). Для сервера `products` — параметр подсказки, а
    не фильтра, и пустой ответ по нему невозможен.
11. **Мок отдаёт ссылки на свои константы, а не копии.** `mockGetBccCategories` возвращает сам
    массив (`services/mocks/bcc.ts:226`), `mockGetBccHistory` — `slice` исходного (`:261`).
    Соседний домен на этом месте ставит `structuredClone`
    (`services/mocks/notifications.ts:424`). Отсюда и возможность страницы дописывать «серверную»
    ленту у себя (БАГ-01), и то, что мутация ответа меняет «сервер» (§18 соглашений).
...
| «сортировка DESC по создания» (`03-api-contract.md:563`) | сервером не выражена: мок не сортирует ничего (`services/mocks/bcc.ts:258-266`), порядок держится `unshift`-ами (`:364`, `:386`), а поля времени точнее суток у строки нет (`types/bcc.ts:24`) — события одного дня не упорядочены ничем |
...
| `subject`/`body` строками у `send` (`03-api-contract.md:573-574`) | на проводе `TranslatedString` — `toTranslatedString(...)` в обоих полях (`services/bccService.ts:39-40`) |
| «Одновременно создаётся N × M row'ов в history со `status: 'sent'`, `source: 'BCC Tool'`» как обязанность сервера (`03-api-contract.md:585`) | сервером не создаётся ни одной: `mockSendBccRequest` ленту не трогает (`services/mocks/bcc.ts:310-335`), строки сочиняет браузер (`BccRequestPage.vue:390-413`, запись `:292`, `:293`) — БАГ-01, БАГ-11 |
| `requestId` формата `req-###` (`03-api-contract.md:581`) | три формата на одно поле: мок отдаёт `req-${Date.now()}` (`services/mocks/bcc.ts:334`), клиент подставляет свой `req-NNN` (`BccRequestPage.vue:274`), схема ограничений не ставит — `String(50)` (`backend/app/modules/bcc/shared/models.py:54-56`) — БАГ-02 |
...
| ответ `log` = `{ requestId: string; events: BccRequest[] }`, «массив созданных строк, чтобы клиент сразу подложил в `history`» (`03-api-contract.md:600`) | ответ несёт только `requestId` (`services/bccService.ts:56`, `services/mocks/bcc.ts:337-343`), строки сочиняет клиент — БАГ-11, БАГ-02. Возвращать ли массив — строка владельцу |
...
| «422, если `price <= 0`» (`03-api-contract.md:607`) | не реализовано нигде: `mockAcceptResponse` цену не проверяет (`services/mocks/bcc.ts:345-370`), клиент проверяет только `NaN` (`BccRequestPage.vue:349`); `VALIDATION_ERROR` ядро объявляет (`backend/app/core/exceptions.py:23-27`), домен его не бросает — роутов нет |
...
| отправка: «сервер создаёт N × M строк» — не создаёт ни одной | «Чего в домене нет», строка 12; раздел `POST /api/bcc/send`, форма ответа (БАГ-11) |
...
| логирование: отправка и логирование обязаны создавать строки одинаково | раздел `POST /api/bcc/log`, абзац о `createEventRows` |
...
| **осталось** · что входит в одну транзакцию отправки и что делать, если письмо ушло, а строки не записались | графа 8; решение владельца (БАГ-01, БАГ-11) |
...
| **осталось** · кому принадлежат перечень единиц цены и перечень источников запроса | графа 1; решение владельца (БАГ-07) |
...
| **осталось** · остаётся ли отдельный след у отправки и приёма цены и кто автор записи | графа 3; решение владельца |
| **осталось** · нужно ли право на отправку письма от лица компании и на приём цены, помимо фича-флага | графа 7; решение владельца |
| **осталось** · рождает ли отправка уведомление и кому адресовано `supplier_response` | графа 2; решение владельца (БАГ-10) |
| **осталось** · есть ли у строки события срок жизни и кто её удаляет | графа 5 (3); «Чего в домене нет», строка 9; решение владельца |
| **осталось** · обновляет ли приём ответа `lastBccDate` и `priceHistory` поставщика | графа 5 (4); «Чего в домене нет», строка 19; решение владельца |
| **осталось** · возвращают ли `send` и `log` массив созданных событий и в какой валюте хранится цена | «Чего в домене нет», строка 16; раздел `POST /api/bcc/events/:eventId/response`, абзац о валюте; решение владельца |
| **осталось** · подключать ли написанный слайс отправки к HTTP и кто пишет схемы, репозиторий и роут | шапка файла, абзац о доменном слайсе; решение владельца |

Находки про код домена —
[`contract-sync-bcc-bugs.md`](../../plans/bugs/contract-sync-bcc-bugs.md), БАГ-01…БАГ-11. Код этой
работой не тронут.
