...
`Supplier` — 17 полей (`frontend_vue/src/types/supplier.ts:12-31`).
...
**Осталось (пробелы аудита, б, в, г):** какой из двух экспортов настоящий, и вместе с ним —
набор колонок, обязательность `Content-Disposition` и стриминга (ни того, ни другого в коде нет
нигде), и объём выборки: прежний контракт обещал «всё, что подпадает под фильтры», а кнопка
экспортирует только текущую страницу
(`frontend_vue/src/views/admin/suppliers/SuppliersListPage.vue:206`). Строка владельцу
...
**Осталось (б):** второй журнал `history` — кто его пишет и где хранит (правило домена 4). Строка
владельцу `suppliers · Запись в аудит-лог · зачем у поставщика второй журнал`.
...
15 общих полей в строке списка (`frontend_vue/src/services/mocks/suppliers.ts:429-447`) — правило
домена 6.
...
**Осталось (г):** формат `oldValue`/`newValue` в записи аудита. Прежний контракт требовал
JSON-строки с ключами изменённых полей; мок хранит там человекочитаемые куски —
...
(`frontend_vue/src/services/mocks/notifications.ts:667-669`) — БАГ-02. **Осталось:** рождают ли
событие создание поставщика, смена статуса и блокировка, кому уведомление адресовано и почему
повтор не рождает второго — строка владельцу `suppliers · События и уведомления`.
...
**Осталось:** формат `oldValue`/`newValue`, уникальность `entryId`, назначение второго журнала
`history` — три строки владельцу `suppliers · Запись в аудит-лог`.
...
**Осталось:** владелец каждого из трёх справочников, закрыт ли список типов адреса и какой из двух
экспортов настоящий — строки владельцу
...
функция, которая проверяет, не существуют обе. **Осталось:** какое право нужно на создание, правку,
чтение поля и удаление записи аудита — строка владельцу
`suppliers · Права — в какой функции проверяются`. Почему это сквозная обязанность, где стоит
заглушка `check_permission` и какой код несёт работающий отказ — §6 соглашений.
...
транзакцией записать изменение и запись аудита, `PATCH /status` — то же самое. **Осталось:** нужен
ли `Idempotency-Key` созданию и `If-Match` карточке — строка владельцу
`suppliers · Транзакционность и идемпотентность`; общие правила — §11 соглашений.
