...
5. **Кастомные поля** — что делать со значениями у товаров, когда `PUT /api/categories/:id/fields`
   ...
   **вопрос владельцу, куст 5, вопрос 1**. Замер, из-за которого он не откладывается: схема
   удаление запрещает (`product_field_values.field_id` объявлен `ondelete="RESTRICT"`), то есть
   сегодня операция упала бы ошибкой, которую никто не показывает.
