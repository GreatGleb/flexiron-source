...
- **уникальности имени нет нигде**: у `Category` (`models.py:14-56`) `UniqueConstraint` нет
  вовсе, единственный в модуле — `uq_product_field_value` (`models.py:210-214`). Требовать её —
  решение, а не наблюдение; сейчас повторный POST с тем же телом создаёт вторую категорию.
...
`ondelete="RESTRICT"` (миграция `:32`; объявление колонки — `models.py:26-31`, сам `ondelete`
на `:28`), и мок ведёт себя так же (`CATEGORY_HAS_CHILDREN`, `mocks/categories.ts:1481`).
А ORM-отношение того же внешнего ключа объявлено с каскадом:
...
12. **ORM-каскад `children` противоречит `RESTRICT` того же внешнего ключа — это пробел, а не
    деталь реализации.** `categories.parent_id` — `ondelete="RESTRICT"` (`models.py:28`,
    объявление колонки `:26-31`; миграция `:32`), то есть удаление категории с потомками обязано
    быть отвергнуто; отношение `children` того же FK — `cascade="all, delete-orphan"`
    (`models.py:44-47`), то есть потомков
...
9. **строки в `00-решения-владельца.md` пока нет:** какое из двух объявлений удаления потомков
   верно — `ondelete="RESTRICT"` на `categories.parent_id` (`models.py:28`, миграция `:32`)
