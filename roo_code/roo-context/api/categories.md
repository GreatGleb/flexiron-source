...
Ещё два кода прежний контракт обещал, и в коде их нет ни одного — см. «Чего в домене нет».
...
Ошибки — три кода каталога выше. Оба запрета уже выражены схемой: `products.category_id` и
`categories.parent_id` объявлены `ondelete="RESTRICT"`
(`backend/alembic/versions/25245d4bf874_phase_3_categories_products.py:32` и `:62`), то есть у
...
3. **Права** — какое право требуется на создание, правку и удаление категории и одно ли оно с
   правом на товары — **осталось**.
