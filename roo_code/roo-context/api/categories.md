...
[`plans/api/audit/categories.md`](../../plans/api/audit/categories.md). Находки про код (восемь,
...
`backend/alembic/versions/25245d4bf874_phase_3_categories_products.py:27-53`. Три расхождения
описаны ниже в «Правилах домена» (§1 — многоязычные имена в `String(255)`, §5 — производные
значения колонками) и обязательны к решению до первого слайса.
...
существует, `mocks/categories.ts:1479`) в поле `code`, а не текст в `message`.
...
|---|---|---|
| `CATEGORY_HAS_PRODUCTS` | у категории есть товары (`:1480`) | `categories.toast_error_delete_has_products` (`i18n/admin/categories.ts:61`, en `:125`, lt `:189`) |
| `CATEGORY_HAS_CHILDREN` | у категории есть потомки (`:1481`) | `..._has_children` (`:62`, `:126`, `:190`) |
| `CATEGORY_NOT_FOUND` | категории нет (`:1479`) | общий `categories.toast_error` (`useCategories.ts:49`) |

Ни один из трёх не является подстрокой другого
...
Каскадного удаления потомков нет и быть не должно: категория с потомками отвергается.
...
с `ondelete="CASCADE"` (`models.py:19-24`, `:64-69`; миграция `:30`, `:44`). Выборка обязана
...
| `404 CATEGORY_NOT_FOUND` у `GET /api/categories/:id` (`:926`) | мок бросает **текст** `Category ${id} not found` (`mocks/categories.ts:1419`), и он же показывается пользователю (`useCategoryCard.ts:92`) — кода на этом пути нет ни одного (находка 4) |
...
Восемь строк домена в
[`00-решения-владельца.md`](../../plans/api/audit/00-решения-владельца.md) (раздел
«categories — аудит 2026-09-04»), каждая отмечена в тексте выше словом **осталось**:
