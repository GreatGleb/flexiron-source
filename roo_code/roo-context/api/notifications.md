...
Реализация: `services/notificationsService.ts:9` (`getNotifications`) · потребитель
...
Реализация: `services/notificationsService.ts:21` (`getUnreadCount`) · потребители
...
Реализация: `services/notificationsService.ts:25` (`markAsRead`) · потребитель
...
Реализация: `services/notificationsService.ts:29` (`markAllAsRead`) · потребители
...
2. **Восьмой тип, `reserve_expiring`, триггера не имеет — и это осознанный отказ, а не пропуск.**
   ...
   ли броне срок — решение владельца.
...
`server_default="{}"` (`:29-30`). Кому принадлежат интервал опроса и глубина ленты — **нигде**,
строка владельцу.
...
| пробел аудита | где |
|---|---|
| `GET /api/notifications` (а) чем ограничена выборка ленты | **осталось** — решение владельца, `## notifications`, строка «Мультиарендность»; наблюдение целиком в «Обязанности сервера», графа 6; БАГ-01 |
...
| (б) право отметить чужое уведомление нигде не проверяется | **осталось** — решение владельца, строка «Права»; наблюдение в графе 7 |
...
поднял не как пробел раздела, а как графу «Обязанности сервера»; они тоже **осталось**: чем сервер
отличит событие от повтора после перезапуска (графа 2), нужен ли броне срок для `reserve_expiring`
(графа 2 и «Правила домена», пункт 2), доставляется ли уведомление вторым каналом и подписывается
ли пользователь на типы (графа 5), сколько живёт запись (графа 5), кто владеет переводами текстов
(графа 5, пункт 1). Итого десять строк — ровно те, что стоят в разделе `## notifications` файла
решений.
...
[`03-api-contract.md`](../03-api-contract.md) (раздел `# Admin — Notifications`, строки 2860–2995),
которое здесь не воспроизведено, названо вместе с тем, чем оно опровергнуто.
...
| пример ответа с `"total": 18` (строка 2938) | сидов в моке двадцать: `grep -c "    id: 'notif-" frontend_vue/src/services/mocks/notifications.ts` → 20. Пример убран целиком: форма задаётся типом `PaginatedResponse<Notification>` (§13), а число сидов — свойство демо-данных, которому в контракте места нет |
| «Response 200: `number` (без `ApiResponse`-обёртки? Формат уточнить)» (строка 2955) | вопрос снят кодом: `unwrap()` снимает конверт для любого тела с ключом `success`, особого случая для этого пути нет (§1, `services/api.ts:128-137`). На проводе `ApiResponse<number>` |
| «Возвращает количество непрочитанных уведомлений **для текущего пользователя**» и «Отмечает все уведомления **текущего пользователя**» (строки 2965, 2987) | понятия пользователя в реализации нет: мок считает по всему стору (`mocks/notifications.ts:433`) и переписывает весь массив (`:444`), клиент не шлёт ни идентификатора, ни заголовка (`services/notificationsService.ts:21`, `:29`). Требование остаётся верным по схеме (`models.py:22-27`), но как **строка владельцу** и БАГ-01, а не как описание существующего |
| «Клиент после успеха декрементирует локальный `unreadCount`» (строка 2979) | делает это **безусловно**, а не после успеха: `Math.max(0, unreadCount.value - 1)` (`useNotifications.ts:58`) выполняется и когда сервер ничего не изменил, потому что ошибка проглочена (`:59-61`) |
