...
`Настройки → Уведомления` (П54, [§10.2](00-conventions.md)). **Осталось** две: сколько живёт запись
(графа 5) и кто владеет переводами текстов (графа 5, пункт 1). Итого десять строк — ровно те, что стоят в разделе `## notifications` файла
решений.
