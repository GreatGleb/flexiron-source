...
Сюда же по решению владельца П34 относится **ширина реза по умолчанию** — значение, которым домен
владеет, но которого в коде ещё нет ни на одной стороне: раздел «Ширина реза по умолчанию» ниже.
...
**Статус:** спроектировано — значения нет ни во фронте, ни на сервере, и ни одного эндпоинта этот
раздел не добавляет.

**Владелец — настройки арендатора, это решение владельца П34**
([`audit/00-решения-владельца.md`](../../plans/api/audit/00-решения-владельца.md), раздел «П34 ·
Ширина реза по умолчанию — в настройки, с переопределением на месте»). Причина названа там же: у
разного оборудования ширина реза разная, поэтому константой в коде она быть не может — в отличие от
размера страницы (П20) и единицы по умолчанию (П26), которые владелец оставил коду.

**Сегодня это литерал.** `const form = reactive({ kerfMm: 3, … })`
(`frontend_vue/src/composables/useWarehouseCutting.ts:172`) — единственный источник начального
значения поля «Ширина реза (мм)» (`views/admin/warehouse/WarehouseCuttingPage.vue:489-498`). В
настройках его нет ни в одном из пяти мест, где оно могло бы быть:
`grep -ci kerf frontend_vue/src/types/settings.ts frontend_vue/src/services/mocks/settings.ts
...
Цена выбора названа прямо: заголовок «Финансовые константы» перестаёт быть точным — ширина реза не
финансовая величина. Путь `/api/settings/constants` при этом не переименовывается: он уже на проводе
и в схеме (`crud/schemas.py:43-51`), а слово «финансовые» живёт только в заголовке раздела.

**На проводе резки не меняется ничего, и это главное в П34.** Переопределение «в каждом резе» уже
существует: `kerfMm` — обязательный ключ тела `POST /api/warehouse/cutting`
([`warehouse.md`](warehouse.md), раздел резки), и клиент заполняет его из формы. Настройка меняет
ровно одно — откуда берётся **начальное** значение этой формы. Поэтому ключ остаётся обязательным,
необязательным не становится и значения «возьми настройку» на проводе не заводится: сервер, которому
прислали число, не должен гадать, своё оно или подставленное.
...
Работа по коду — снять литерал `3` из `useWarehouseCutting.ts:172` и читать значение из стора
настроек. Складская половина этого решения — [`warehouse.md`](warehouse.md), графа 1 обязанностей
сервера.
