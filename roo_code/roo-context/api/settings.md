...
которому сервер режет границу суток и месяца ([§14](00-conventions.md)), и **страну**, которая
перестаёт быть частью строки адреса и хранится **кодом** ISO 3166-1 alpha-2. Страна выбирается из
закрытого списка (`domain/countries.ts`, 249 кодов, из настроек не редактируется), выбор обязан
быть с поиском, и поиск идёт **по названиям на всех языках сразу** плюс по самому коду. Сервер
хранит код и отдаёт код; название выводит клиент по языку читателя.

Ответ — `CompanyInfo`, шесть ключей в camelCase (**до** П62 и П66):
