...
| **решено 2026-09-08 частично (П36)** · правило записи общее: любое изменение любого свойства, автор — `user_id` плюс снимок имени. **Осталось:** заводится ли `settings` десятой сущностью ленты — §9 предупреждает, что «десятой не будет без своей таблицы», а страница логов при этом живёт вкладкой внутри настроек | графа 3; [§9](00-conventions.md) |
