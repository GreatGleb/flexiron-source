...
| **решено 2026-09-08 частично (П36)** · правило записи общее: любое изменение любого свойства, автор — `user_id` плюс снимок имени. **Осталось:** заводится ли `settings` десятым видом сущности. Цена вопроса упала после П38: журнал стал одной таблицей, и десятый вид — это новое значение `entity_type`, а не новая таблица. Довод за — смена матрицы прав меняет доступ всем сразу, и странно, что страница логов живёт вкладкой внутри раздела, изменения которого в неё не попадают | графа 3; [§9](00-conventions.md) |
