...
Две строки владелец не отнёс ни к одной стороне, и они остаются открытыми: **карточка клиента**
(`1 + N + M` запросов) и **финансовая запись** (аплоад до Save). Критерий выдан — «атомарен там,
где полумера опасна», — но примеры названы только два, а достраивать за владельца контракт не
вправе.
