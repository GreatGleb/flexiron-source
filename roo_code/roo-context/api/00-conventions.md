...
Сегодня способа нет: `User(...)` конструируется в одном месте на весь бэкенд
(`backend/app/modules/auth/features/register/repository.py:80`), и регистрация всегда создаёт
**нового арендатора** (`backend/app/modules/auth/features/register/domain.py:98-103`). Список пользователей во фронте при этом
есть и показывается (`types/settings.ts:245`), а пополнить его нечем — среди 24 роутов `settings`
управления пользователями нет.

**Решено:** владелец **сначала заводит запись** — пишет имя и выбирает роль (обычный элемент
матрицы, §6), — а затем получает **ссылку-приглашение** и передаёт её как считает нужным: письмом
(§24) или скопировав и отправив коллеге любым способом. Приглашённый переходит по ссылке и
**ставит себе пароль сам**.

Два следствия, которые из этого вытекают:

- **почта не условие, а способ доставки.** Пока почтовый сервер арендатора не настроен,
  приглашение всё равно работает — ссылку передают руками;
- **запись пользователя существует до его первого входа**, в состоянии «приглашён», с именем и
  ролью и без пароля. Уникальность почты считается парой с арендатором (§4 выше), то есть один и
  тот же адрес может быть приглашён в две разные компании.

Механизм секретных ссылок заводить не нужно — он написан: слайс `magic_link` с `verify_secret_link`
(`backend/app/modules/auth/features/magic_link/action.py:22-36`).
...
### 6.4. Хранение матрицы и дефолт нового элемента

На бэкенде, **отдельно для каждого арендатора**, редактирует владелец и админ. Фронтовая матрица
(`services/mocks/config.ts`) остаётся **только** мок-режиму и источником истины не является.

**Матрица по умолчанию не пустая, и новый элемент не заводится «без прав».** Появился элемент —
права на него сервер **вычисляет** в тот же момент (П33). Правило двухслойное.

Слой первый, базовый: новый элемент видит **только админ**.

Слой второй, поверх базового, — правило 90 %:

| У кого | Что он получает на новый элемент |
|---|---|
| роль или отдельный пользователь, у которого **полные** права — все четыре действия — не меньше чем на 90 % элементов **того же домена** | полные права |
| роль или отдельный пользователь, у которого **какое-то одно** действие есть не меньше чем на 90 % элементов **того же домена** | это же одно действие |

Как это читается — и от чтения зависит формула, а не только слова:

- доля считается **по элементам матрицы внутри одного домена**, а не по матрице целиком;
- «полные права» — все четыре действия `read`, `edit`, `create`, `delete` (6.2);
- действия добираются **поштучно**: роль, у которой `read` есть на 95 % элементов домена, а
  `edit` — на половине, получает на новый элемент `read` и не получает `edit`;
- правило действует на оба уровня 6.1 — и на роль, и на переопределение пользователя;
- срабатывает оно **в момент появления элемента** и кладёт обычные строки `role_permissions` и
  `user_permissions`. Вычисляемого состояния у права нет: дальше оно правится руками, как любое
  другое, и пересчёту при следующем новом элементе не подлежит.
...
**В кусте прав нерешённой осталась одна строка** — «config · какой признак „этого удалять нельзя“
настоящий: колонка на схеме или префикс id». Она стояла в графе «Права», но по существу относится
к защищённым объектам, а не к ролям, поэтому куст «права» её не закрыл — и ни один следующий куст
её не подобрал: она **открыта** до сих пор
([`audit/00-решения-владельца.md`](../../plans/api/audit/00-решения-владельца.md), раздел
«Не отвечено этим кустом»; [`config.md`](config.md), «Пробелы аудита: закрыто и осталось»).

Прежняя редакция этого абзаца утверждала «нерешённым не осталось ничего» и тут же называла эту
самую строку — два утверждения противоречили друг другу с первой же редакции, и заодно с абзацем
«Куст „права“ закрыт 2026-09-07» в конце файла, который отсылал сюда за **тремя** пунктами.

Дефолт прав нового элемента, который контракт до 2026-09-12 числил за владельцем, к этой строке
отношения не имеет: он пришёл из куста «умолчания», решён П33 и внесён в 6.4.
...
**Значение переживает определение.** Поле убрали из набора категории — значения у товаров
**перестают показываться, но сохраняются**; вернули поле в набор — значения снова видны. Тот же
принцип, что П44 для товаров и услуг: архив вместо удаления, только для значения. То же правило
действует для **унаследованных** полей, пропавших при смене родителя категории.

Из этого следует, что `product_field_values.field_id` перестаёт быть `RESTRICT`
(`alembic/versions/25245d4bf874_phase_3_categories_products.py:78`): определение не удаляется
вместе с набором, а помечается убранным, и значение продолжает на него ссылаться.

**Операция закрыта кодом подтверждения, и это новый механизм проекта.** Нажать «ОК» в
предупреждении недостаточно: человеку показывается число затронутых записей («значения у 300
товаров перестанут показываться, но сохранятся») и требуется ввести **секретный код из четырёх
цифр**. Код генерируется с самого начала, живёт в настройках и виден **админу и владельцу**; право
на саму операцию — тоже только у них.

Четыре следствия, из которых три — чтение контракта, а не слово владельца:

- **проверяет код сервер, а не браузер** — иначе защита декоративна: модалку обходят запросом;
- **код принадлежит арендатору**, а не пользователю: он «виден в настройках», то есть один на
  компанию;
- **видимость кода — право на поле, а не на раздел**: страницу настроек открывают и другие роли,
  а код на ней видеть не должны, и матрица прав такого случая сегодня не знает (§6.2);
- **область действия — только эта операция**; распространение кода на другие разрушительные
  действия будет отдельным решением, но механизм у него уже будет.

**Код постоянный и не перевыпускается (решено 2026-09-11).** Он лежит в настройках и админу виден
всегда — секретом от тех, у кого есть право, он и не был. Разжаловали админа: знание кода ему не
поможет, потому что операцию остановит **право**, а не код. Отсюда порядок проверки на сервере:
**сперва право, потом код**. Код без права не даёт ничего — значит его утечка не событие
безопасности; право без кода не даёт ничего — значит случайное нажатие не проходит.
...
## 9. Аудит-лог: одна таблица, девять видов сущности

**Решено 2026-09-08 (П38): журнал — одна общая таблица** `audit_entries` с колонками
`entity_type` и `entity_id`, ключ **UUIDv7**. Прежний порядок «таблица на сущность» отменён.
...
Историю держат **девять** видов сущности: `product`, `order`, `client`, `supplier`, `batch`,
`stock`, `offcut`, `movement`, `deficit`. Перечень замкнут (`types/audit.ts:4-14`, тот же список
константой `:16-26`, маршруты карточек `:35-45`); десятый вид теперь стоит не новой таблицы, а
нового значения `entity_type` — но заводить ли его, всё равно решает владелец.

**Ключ записи глобально уникален** (UUIDv7), поэтому тройка `entityType + entityId + entryId`
перестаёт быть адресом и остаётся способом ключевать строку в интерфейсе. Удаление по-прежнему
идёт в эндпоинт своей сущности — девять эндпоинтов это про то, **кто вправе** удалять, а не про
адрес, и `entityId` в пути превращается в проверку принадлежности:
...
**Решено 2026-09-08 (П36, П37).**

- **Пишется любое изменение любого свойства сущности** — не только правка поля формой, но и
  доменные операции, меняющие состояние: у заказа это, в том числе, движения, потому что
  количество товара иначе не меняется. Совпадает с исходным ТЗ: «Регистрация любого действия в
  системе (создание, редактирование, удаление, смена статуса)»
  (`toDo/Flexiron_ERP_CRM.md:184`), состав записи оттуда же (`:185`) — кто, когда, какое поле,
  было → стало.
  Объём работы измерим на заказе, единственном пишущем домене: мутирующих операций **24**, след
  оставляют **6** (`mockPatchOrderStatus`, `mockUpdateOrderItem`, `mockUpdateOrderService`,
  `mockCorrectOrderLine`, `mockCancelShipment`, `mockCreateReturn`). Отгрузка следа не оставляет,
  а отмена отгрузки оставляет — по П36 обязаны обе. У остальных восьми доменов писателя нет
...
- **Записи журнала удаляет владелец, и только он** (П8). Требование ТЗ «логи не могут быть
  ...
  устаревшим 2026-09-08 — это сознательная отмена, а не расхождение реализации с ТЗ. Девять
  существующих эндпоинтов удаления остаются и закрываются правом владельца. **Мета-записи об
  удалении не будет:** неудаляемый след лишил бы право смысла, а удаляемый — тем более.
- **Признак `sensitive` заводится всем девяти видам единообразно**, чаще всего `null` — по
  правилу «Present always with `null` rather than sometimes absent» (`types/order.ts:598-605`).
  Он обязателен, потому что П5 требует от сервера **вырезать** помеченные записи у того, у кого
  нет права; чтобы вырезать, надо пометить.
- **Просмотр в журнал не пишется** — затея нецелесообразна: записей она даёт больше, чем все
  изменения вместе, а пользы нет, поскольку по П5 и П17 сервер вырезает недоступные числа тому,
  у кого нет права, и увидеть чужое нельзя.

**Решено 2026-09-08 (П39–П42) — два яруса журнала.**

Три ответа вместе задают деление, которого в проекте не было. Критерий не «важность», а **форма
события**: укладывается в строку ленты — идёт в ленту; не укладывается или не предназначено
пользователю — в серверный лог.

| Ярус | Что туда идёт | Кто видит |
|---|---|---|
| **Лента** `audit_entries` | события формы «сущность · свойство · было → стало · автор» — **десять** видов сущности, включая настройки | пользователь |
| **Серверные логи** | события без этой формы (вход, выход, неудачная попытка, переход по секретной ссылке) и то, чего пользователю видеть не нужно (мета-запись об удалении записи журнала) | администратор сервера |

- **Смена настроек — десятый вид сущности ленты** (П42): у неё есть всё, что нужно строке —
  свойство, старое значение, новое, автор. Замкнутый перечень `AuditEntityType`
  (`types/audit.ts:5-14`) расширяется до десяти; после П38 это новое значение `entity_type`, а не
  новая таблица. Довод: смена матрицы прав меняет доступ всем сразу, а страница логов живёт
  вкладкой внутри того самого раздела (`router/index.ts:391-396`).
- **Журнал безопасности — серверный** (П41): у входа и выхода нет ни `entityType`, ни
  `oldValue`, ни `newValue`, в строку ленты они не укладываются. Таблицы под него сегодня не
  существует.
- **Мета-запись об удалении есть, но не в ленте** (П40): след остаётся в серверных логах, на
  фронте не показывается. Право владельца удалять смысла не теряет — удаляется видимое, отметка
  остаётся там, куда пользователь не смотрит.
- **`oldValue` / `newValue` — готовая строка: форматирует сервер, клиент печатает** (П39).
  Уточнение, прочитанное при записи: строка собирается **при чтении**, в локаль запросившего, а
...
**Решено 2026-09-09 (П56): повтор после перезапуска отличает база, а не память процесса.** У
события заводится отметка «уже уведомили», и сервер её проверяет. Сегодня память просрочки живёт
в процессе (`mocks/finance.ts:63`), колонки под отметку на схеме нет — это новая работа, а не
уточнение существующей. Без неё каждый перезапуск даёт пачку повторов о том, что человек уже
видел, и колокольчик перестают читать.
...
### 10.1. Перечень типов: восемь становится пятнадцатью (П51, решено 2026-09-09)

Тринадцать строк аудита спрашивали в разных доменах одно и то же — рождает ли уведомление то или
это, — и ответ был один: нет, потому что нет типа. Перечень замкнут типом
(`types/notifications.ts:3-11`, восемь значений). Владелец назвал нужными **все семь** кандидатов,
и перечень становится пятнадцатизначным:

| Новый тип | Повод | Кому |
|---|---|---|
| заказ отгружен | отгрузка (сегодня меняется статус, отгрузка отдельно не звучит) | продажи, менеджер клиента |
| счёт выставлен | выставление счёта | бухгалтерия |
| возврат товара | оформлен возврат | склад, продажи |
| матрица прав изменена | правка матрицы — доступ меняется у всех сразу | владелец, админ |
| цена услуги изменена при незакрытых заказах | правка цены услуги, стоящей в незакрытом заказе | продажи |
| поставщик заблокирован | блокировка поставщика | закупки |
| партия просрочена | партия перешла границу годности | склад |

Правило отбора названо вместе с ответом и действует на будущее: **уведомление заводится там, где
человек обязан узнать сразу, а не найти потом.** Всё прочее изменение и так пишется в журнал
(§9, П36), и каждый лишний тип — ещё один колокольчик, который перестают читать.

Что из этого **не** заводится, потому что в перечень не вошло: создание, правка и удаление
категории и замена набора её полей; смена НДС, валюты по умолчанию и набора статусов заказа;
создание клиента, перевод его в `inactive`, правка условий оплаты; создание поставщика и обычная
смена его статуса; привязка документа к сущности; создание, правка и удаление товара; сама
отправка BCC-запроса; списание, резка и движение на складе; корректировка выданного документа;
закрытие счёта клиента и смена статуса исходящего платежа помимо просрочки.

### 10.2. Два канала: лента и почта, и ведут они себя по-разному (П54, решено 2026-09-09)

Появляется вкладка **`Настройки → Уведомления`**: строки — типы уведомлений (все, что есть в
системе), столбцы — каналы **«На сайте»** и **«На почту»**, в пересечениях галочки. Таблица
подписок привязана **к пользователю**: у каждого свои галочки, и там же у каждого указывается
почта — по умолчанию та, что привязана к нему, но её можно изменить.

**Разница каналов — правило, а не деталь реализации:**

- **Сайт.** Записи создаются **всегда**, независимо от галочек: галочка «на сайте» есть фильтр
  показа, а не разрешение на запись. Снятая галочка ничего не удаляет — вернули её, и старые
  непрочитанные снова видны в ленте и читаются; теряется только то, что человек сам отметил
  прочитанным. Это продолжение П10, где персональный фильтр назван настройкой, а не правом.
- **Почта.** Письмо либо ушло в момент события, либо не ушло. Галочка решает **именно в этот
  ...
  **Почтовая галочка работает вперёд, сайтовая — и назад тоже.**

Работа, которую это назначает: таблица подписок «пользователь × тип × канал», фильтр по ней в
выдаче ленты, отметка «отправлено письмом» у события и отправка через почтовый сервер арендатора
(§24, П53). Сегодня нет ни одного из четырёх.

### 10.3. Срок брони — три рабочих дня, и «рабочий день» в проекте не определён (П52)

Восьмой тип `reserve_expiring` объявлен и не срабатывает никогда, потому что истекать нечему:
у `StockReservation` нет ни срока, ни даты окончания (`types/warehouse.ts:625-634`), а
`Batch.expiresAt` — годность металла, а не граница брони (`:107`).

**Решено 2026-09-09: срок есть — три рабочих дня по умолчанию, значение живёт в настройках** (П19:
что настраивается на странице настроек, хранится в БД и спрашивается у сервера). Восьмой тип
оживает, и снимается его нынешнее положение «объявлен, но притворяется».

**Следствие, которого в проекте нет ни в каком виде: «рабочий день» требует календаря.** Ни
выходных, ни праздников, ни производственного календаря арендатора —
`grep -rln "workday\|working_day\|holiday\|businessDay"` по `frontend_vue/src` и `backend/app`
пусто. Умолчание контракта: **суббота и воскресенье в срок не входят**; праздники — либо календарь
страны арендатора, либо их отсутствие, и выбор между этими двумя остаётся работой реализации, а не
пробелом контракта.

### 10.4. Тревоги дашборда — проекция ленты, а не свой механизм (П55, решено 2026-09-09)

То, что тревожит, — **просроченные платежи, нехватка под подтверждённый заказ, партии на исходе** —
рождает уведомления; эти уведомления помечаются **особым флагом «требует действия сейчас»**, и блок
`alerts` на дашборде показывает помеченные. Одно событие, одна запись, два места показа.

Два уточнения из замера. Первое: два повода из трёх тип уже имеют — `payment_overdue` и
`stock_deficit`; новое здесь не тип, а флаг и более узкий повод — нехватка именно **под
подтверждённый заказ**, а не любая. Второе: сегодняшний `AlertItem` это `type`, `description` и
`status` без идентификатора и без ссылки на сущность (`types/analytics.ts:31-37`), то есть по
тревоге некуда перейти; как проекция уведомления она обязана нести то же, что несёт строка ленты.
...
**Решено 2026-09-09 (П46): ключ живёт сутки, область действия — «ключ + операция».** Кэш
ответов хранится **24 часа** — этого хватает на повтор после обрыва связи. «Тем же самым»
считается пара «ключ и операция, на которую он послан»: один и тот же ключ на отгрузке и на
платеже — два разных случая, а не один, и второй запрос выполняется, а не получает чужой ответ.

Это закрывает два пробела, которые сервер обязан задать иначе, чем сегодня задаёт мок: его кэш
живёт в `Map` на процесс — **без срока и без привязки к пути** (`mocks/index.ts:260`), то есть
один ключ, посланный на отгрузку и на платёж, вернул бы первый ответ на оба. Прежний контракт
обещал кэш 24 ч и тем самым угадал срок; области действия ключа не задавал ни он, ни §3
[контракта заказов](../../plans/orders/orders-backend-contract.md).

Ключ — не единственная защита от двойной отправки и не главная: клиент обязан замораживать кнопку
до ответа сервера (П47, §15).
...
### 12.1. Ввод — один язык, чтение — тот, что есть (П64, решено 2026-09-10)

Правило владельца, заданное как **универсальное и действующее везде**, а не в одном домене:

- **при создании вводится один язык** — тот, на котором человек работает; трёх обязательных
  названий не требуется нигде, и услуги ничем не отличаются от товаров и категорий;
- **при показе берётся текущий язык интерфейса, а если на нём не сохранено — любой сохранённый.**
  Есть только литовский — показывается литовский, а не пустота и не ключ.

Для сервера это ничего не меняет в хранении: он по-прежнему хранит и возвращает все три ключа без
изменений, а пустая строка означает «на этом языке не заполняли». Правило про **чтение**, и
исполняет его тот, кто показывает.

Замер на момент решения: правило уже живёт во фронте, но россыпью. Цепочка
...
- **Границу суток и месяца сервер режет по часовому поясу компании (П62, решено 2026-09-10).**
  ...
  экране местное: Литва это UTC+2/+3, и заказ, оформленный первого числа в час ночи, по Гринвичу
  попадает в **прошлый** месяц — в оборот текущего он не войдёт, и человек не поймёт почему.
  Часовой пояс — **отдельное поле**; страны, рядом с которой его предлагалось положить, в
  реквизитах сегодня нет — `CompanyInfo` это `name`, `legalAddress`, `vatCode`, `bankName`,
  `bankAccount`, `logoUrl` (`types/settings.ts:4-11`), и на схеме то же
  (`backend/app/modules/settings/shared/models.py:24-29`), страна живёт внутри строки адреса.
- **У компании есть страна, и хранится она кодом (П66, решено 2026-09-10).** Поле своё, а не часть
  адреса: человек выбирает страну **из закрытого списка**, компании присваивается код ISO 3166-1
  alpha-2. Справочник уже написан и объясняет сам себя — `domain/countries.ts:1-18`, 249 кодов, с
  доводом «у свободного текста „Литва“, „Lietuva“, „LT“, „lt.“ — четыре разных значения и одна
  страна»; список **из настроек не редактируется**, потому что состав задаёт ISO, а не компания.
  Клиент так и устроен уже сегодня (`types/client.ts:28`).
  **Выбор обязан быть с поиском, и поиск идёт по названиям на всех языках сразу** — русский
  интерфейс обязан находить Литву по `Lietuva` и по `LT`. Это работа, а не настройка: поискового
  селекта в проекте нет ни одного (страну рисует `CustomSelect`, 99 строк без поля ввода), а
  названия сегодня собираются на одной локали — `countryLabel(code, locale)` и
  `countryOptions(locale)` (`domain/countries.ts:309-315`, `:351-355`) отдают по одной подписи на
  код. Название **выводится при показе** и зависит от языка читателя; хранится только код.
  **Правило общее для всех трёх мест, где страна есть (П67, решено 2026-09-10): компания, клиент
  и поставщик.** Свободного текста в поле страны не остаётся нигде. Клиент так устроен уже, и
...
**Решено 2026-09-09 (П43): атомарность выборочная.** Одной транзакцией применяется Save там, где
полумера опасна, — **у заказа и у склада**: сервер принимает изменение целиком и применяет его
целиком либо не применяет вовсе. У **справочников и настроек** остаётся сегодняшнее поведение, и
контракт называет его прямо, а не умалчивает: череда независимых запросов, частичное сохранение
возможно, порядок не задан. Это относится к `settings`, `config`, `categories`, а также к
`products`, `suppliers` и `services`, у которых Save и так один PATCH.

Две строки в пять вопросов куста не попали и были заданы отдельно; **добор 2026-09-09 (П49):
и карточка клиента, и финансовая запись остаются как есть.** У клиента расходится список
контактов и взаимодействий, а не деньги и не остатки; у финансовой записи неатомарна ровно связка
«файл плюс запись» (аплоад уходит до Save), а осиротевший файл закрыт уборщиком черновиков (П31),
не транзакцией. **Перечень атомарного этим закрыт: заказ и склад, и больше ничего.**

**Решено 2026-09-09 (П47): двойная отправка гасится и на клиенте.** Правило сквозное и
обязательное на всех страницах: как только фронтенд отправил данные, кнопка заморожена и второй
раз нажать нельзя, пока не придёт ответ сервера. Разморозка — только после ответа, и порядок
именно такой: сначала человек видит, что операция завершилась (окно закрылось, возврат отправлен),
и лишь затем кнопка снова живая — а чаще снова неактивная, потому что форма опустела.

Ключ идемпотентности (§11) этого не заменяет: он прикрывает пять необратимых операций из 175
вызовов, а замораживать кнопку обязаны все отправки без исключения. Сегодня так делают **22 файла
из 56**, в которых есть `<button>`, и флаг занятости заведён в **18 композаблах из 51**. Отсюда
следствие для сервера: ответ обязан быть достаточным, чтобы клиент разморозил кнопку и обновил
...
- **Повтор файла отсеивает клиент, сервер о нём не знает (П50, решено 2026-09-09).** Один и тот же
  файл, поданный в форму дважды, второй раз в отправку не идёт; на сервере не появляется ни хеша
  содержимого (`core/uploads/models.py:11-38`), ни проверки дубля, ни кода ошибки — повторная
  загрузка остаётся законной операцией. Сравнивать клиент может только то, что даёт `File`: **имя,
  размер и дату изменения**; тот же файл под другим именем отсев не поймает, и это принято
...
- **Ограничениями владеет сервер, и фронт их не знает — а с П30 обязан узнавать.**
...
**Решено 2026-09-07** (П30 — [файл решений](../../plans/api/audit/00-решения-владельца.md)):
**сервер отдаёт свои ограничения клиенту.** Владение остаётся у сервера — отвергает файл по размеру
и типу он и только он, — но значения перестают быть его внутренним делом: клиент получает
предельный размер и белый список MIME от сервера и по ним подсказывает выбор. Константы во фронте
этим отменяются: `accept` собирается из полученного списка, а не пишется литералом в шаблоне, и
отсев по размеру до отправки идёт по полученному значению. Отдавать сегодня нечем — эндпоинта
настроек аплоада нет, — поэтому раздел чтения ограничений написан вперёд, с меткой
`Статус: спроектировано`: `GET /api/uploads/limits` в [`uploads.md`](uploads.md). Подсказка
проверкой при этом не становится: `accept` фильтрует только диалог выбора и перетаскивания не
касается, отказ всё равно выносит сервер.
...
- **Расхождения «фронт считает — схема хранит» разрешены 2026-09-11 (П68): колонок нет,
  ...
  пишется даже при отправке запроса цен), то есть они существуют и всегда врут. Отвергнут вариант
  «колонки остаются, сервер обновляет их событием»: каждое место, забывшее обновить, даёт
  молчаливое расхождение. Понадобится скорость — кэш заводится отдельно и осознанно, а колонкой в
...
### 17.1. Исключение одно: чужие данные копируются колонкой и обновляются событием (П72)

**Решено 2026-09-11 вместе с П68 и намеренно ему противоположно.** Различие — **где лежит
источник**:

| источник данных | как | почему |
|---|---|---|
| **свой модуль** | колонки нет, считается при чтении (П68) | считать дёшево, а хранимое устаревает молча |
| **чужой модуль** | колонка есть, значение копируется, **событие обновляет копию** | считать при чтении значило бы ходить в чужой домен на каждый запрос |

Под исключение попадают два случая, названные владельцем поимённо:
...
Цена решения, которую контракт обязан назвать вслух: **каждое место, рождающее событие, обязано
обновить копию**, и пропущенное место даёт расхождение, которое ничем не видно. Отсюда требование к
доменному файлу: у каждой такой колонки перечислены **все** события, которые её двигают. Для
`priceHistory` это как минимум запись прайса и приём ответа BCC.

**Пересчёт производного — обязанность того, кто меняет вход, а не того, кто читает выход
(П57, решено 2026-09-09).** Разобрано на самом дорогом случае: нехватка на складе выводится из
пары «остаток и порог `minStock`», порог принадлежит карточке товара, а записи нехватки заводят
складские операции. Правка порога обязывает сервер пересчитать нехватку **сразу** — завести её,
если остаток ниже нового порога, и закрыть, если он снова выше. Иначе производное значение живёт
двумя жизнями: на экране остатков оно верное (считается при чтении,
`services/mocks/warehouse.ts:474`), а в таблице нехваток и в ленте уведомлений — прежнее, пока со
складом что-нибудь не случится.
...
- **`entryId` уникален глобально** — решено 2026-09-08 (П38): журнал стал одной таблицей, ключ
  ...
  `entryId` во всём фронте используется дважды — как `:key` во `v-for` и как аргумент удаления, —
  и на экран не печатается: колонки журнала это дата, пользователь, свойство, старое и новое
  значение. Номер BCC, наоборот, виден бейджем `#7`, поэтому там счётчик (П21).
...
## 22. Удаление: у товаров и услуг — архив, у справочников — отказ

**Решено 2026-09-09 (П44).** Правило делит удаляемое надвое по одному признаку: попадает ли
запись в исторические документы.

**Товар и услуга — мягкое удаление.** Запись помечается архивной и остаётся в базе: заказ
двухлетней давности обязан читаться как был. Видимость архивной записи задана точно:

- в таблице товаров (услуг) её больше нет;
- ни в одном выборе она не предлагается — ни в заказе, ни в запросе поставщику, ни где-либо ещё;
- **по ссылке из заказа и других таких же старых сущностей она открывается.**

Отсюда две обязанности сервера: списочные эндпоинты по умолчанию не отдают архивные записи, а
`GET /api/products/:id` и `GET /api/services/:id` отдают их наравне с живыми, помечая признаком
архива. Клиенту нужен этот признак, чтобы карточка, открытая из старого заказа, объяснила, почему
запись не предлагается в новых.

**Справочник — отказ.** Единицу измерения или валюту, на которую есть живая ссылка, сервер
удалить не даёт и отвечает отказом. Код ошибки берётся из каталога §2 по правилу «нельзя из-за
связей» — тот же класс, что `CATEGORY_HAS_PRODUCTS`.

**Что это ломает в схеме.** Сегодня политика удаления не выражает ни ту, ни другую половину
правила. Ссылки на товар разложены по трём разным политикам: `RESTRICT` у партий
(`alembic/versions/fd0ecc1269df_phase_9_warehouse.py:30`), `SET NULL` (`:77`), `CASCADE`
(`:93`, `:107`) и `CASCADE` у значений полей
...
**Решено 2026-09-09 (П45).** Все вкладки аналитики и обе сводки sales-crm считаются на **одну
отметку времени**, и сервер присылает её в ответе. **Пользователю её не показывают** — она нужна
как гарантия связности цифр, а не как элемент интерфейса.

Причина: восемь вкладок аналитики — восемь независимых запросов, между которыми приходят новые
заказы и платежи, а sales-crm грузит сводку и два виджета порознь. Без общей отметки на одной
вкладке продано на 100 000, на другой на 100 500, и человек не знает, какой верить. П27 сделал
вопрос острее, а не мягче: период кончается «текущим моментом», и у восьми запросов он разный.

Форма отметки — поле ответа рядом с данными; имени в коде у неё сегодня нет ни в одном домене,
поэтому его задаёт доменный файл, а не общее правило.

## 24. Почта: транспорт написан, место ввода спроектировано, хранилища нет

**Пароль хранится на сервере и шифруется (П59, решено 2026-09-10).** На фронте он не хранится, а
только передаётся — это уже так: контракт домена описывает запись без чтения обратно, пустое поле
означает «не менять», и мок держит секрет на своей стороне (`services/mocks/settings.ts:37-38`). В
базе пароль лежит **зашифрованным**: иначе любой, у кого есть доступ к базе, прочитает почтовый
пароль компании. Вариант «один почтовый сервер на всю систему, арендаторы его не трогают»
отвергнут — письма обязаны уходить с адреса компании.

**Решено 2026-09-09 (П53): почта в системе есть.** Она нужна минимум в трёх местах — BCC (весь
инструмент про письма поставщикам), регистрация (секретная ссылка сегодня живёт во всплывающем
окне и теряется вместе с ним) и уведомления (§10.2).

**Что уже готово, и потому проектировать заново нечего:**

- транспорт — настоящий `smtplib.SMTP` с логином и шифрованием
  (`backend/app/modules/bcc/features/send_request/transport.py`, доменное правило —
  `backend/app/modules/bcc/features/send_request/domain.py:111-124`), и **его никто не вызывает**;
- место ввода — вкладка `Настройки → Почта` (`views/admin/settings/MailSettings.vue`) с полями
  ровно под то, чего ждёт транспорт: хост, порт, шифрование (`none`/`ssl`/`starttls`), логин,
  пароль на запись без чтения обратно, адрес и имя отправителя, кнопка тестового письма;
- контракт клиента — три эндпоинта (`services/settingsService.ts:166`, `:170`, `:179`) и три
  раздела доменного файла под них (`settings.md`, `GET`/`PATCH /api/settings/mail`,
  `POST /api/settings/mail/test`), плюс мок с посевом (`services/mocks/settings.ts:28-38`).

**Чего нет: хранилища.** На бэкенде ни таблицы, ни маршрута — ни одной колонки `smtp`/`mail_` ни в
одной модели, ни одного роута почты среди 24 роутов `settings`. Отсюда формулировка «негде хранить
настройки» верна ровно про базу и неверна про интерфейс.

Работа названа поимённо: хранилище под уже описанные поля, три эндпоинта к нему и вызов уже
написанного транспорта из BCC. Пароль хранится так, как его уже описал контракт: пишется и не
читается обратно, пустое поле означает «не менять».

## 25. Срок хранения записей: сроков нет, и это станет вопросом тарифа

**Решено 2026-09-10 (П61).** Ни у уведомлений, ни у записей журнала, ни у истории BCC срок
хранения **не назначается**: не удаляем ничего. Сегодня так и есть — удаления в этих доменах не
существует ни эндпоинтом, ни функцией мока.

Причина названа вперёд и меняет смысл будущего решения: хранение станет **предметом тарифа** —
клиент платит за него, значит храним; не платит — удаляем. На дорогих планах храним в любом
случае, если объём не окажется слишком велик и слишком дорог. То есть срок хранения будет не
техническим умолчанием, а свойством плана (модуль `billing`), и назначать его сейчас значило бы
записать в контракт то, что тариф потом отменит.

Следствие для базы, которое записывается уже сейчас: раз срока нет, таблицы журнала, уведомлений и
истории BCC обязаны быть готовы к росту без предела — это довод за разбиение по времени, а не за
уборщик. Правило записи журнала (§9, П36 — пишется любое изменение любого свойства) этот рост
умножает.

Не путать с П31: уборщик черновиков файлов (§16) убирает незавершённую загрузку, а не истекшую
запись, и от П61 не зависит.
...
§10 (уведомления), §16 (файлы). Нерешённой в кусте осталась **одна** строка — она названа в §6.10;
счёт «три пункта», стоявший здесь прежде, не сходился ни с §6.10, ни с разделом «Не отвечено
этим кустом» файла решений.

**Куст «настройки и арендаторы» закрыт 2026-09-10.** Двадцать семь строк — 16 графы «Настройки,
которых мок не отслеживает» и 11 «Мультиарендности». Мультиарендность отсеялась почти целиком: §4
уже задал правило на все домены, и от неё осталась одна строка, про пользователей. Девять строк
сняты прежними решениями (П11, П19, П20, П23, П26, П42 и схемой), восемь вопросов заданы владельцу
и отвечены — П58–П65. Обязанности назначены в §4.1 (приглашения), §12.1 (язык ввода и чтения),
§14 (часовой пояс), §24 (шифрование пароля почты), §25 (сроков хранения нет) и в доменных файлах
`settings`, `suppliers`, `config`, `sales-crm`. Открытыми остались три соседа по многосоставным
строкам: чем лента уведомлений ограничена сверху, кто владеет справочником статусов платежа и
перечнем типов документа архива.

**Куст «события и уведомления» закрыт 2026-09-09.** Семнадцать строк графы, заданных в
шестнадцати доменах: пять сняты прежними решениями (П40, П10, П24, §10), пять вопросов заданы
владельцу и отвечены — П51–П56. Обязанности назначены в §10.1 (перечень типов вырос до
пятнадцати), §10.2 (два канала и вкладка подписок), §10.3 (срок брони), §10.4 (тревоги как
проекция ленты), §10 (отметка «уже уведомили» в базе) и §24 (почта). Открытым остался один
сосед по строке, про уведомления не спрашивавший: обязана ли правка `minStock` пересчитывать
дефицит.

**Куст «транзакционность» закрыт 2026-09-09 целиком.** Восемнадцать строк графы
«Транзакционность и идемпотентность», заданных в тринадцати доменах: шесть сняты прежними
решениями (§11, П29, П31), одна оказалась работой, а не решением, пять вопросов заданы владельцу
и отвечены (П43–П48), и ещё три остатка, которых те пять не покрыли, добраны в тот же день —
атомарность карточки клиента и финансовой записи (П49) и повторная загрузка того же файла (П50).
Обязанности назначены в §15 (атомарность, заморозка кнопки), §11 (срок и область ключа),
§16 (отсев повтора файла), §22 (удаление и архив), §23 (отметка времени отчёта) и в доменном
файле `bcc`.
