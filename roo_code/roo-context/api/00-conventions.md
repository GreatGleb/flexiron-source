...
- **Границу суток и месяца сервер режет по часовому поясу компании (П62, решено 2026-09-10).**
  ...
  **Поправка к формулировке решения:** пояс предлагалось положить «рядом со страной», а страны в
  реквизитах нет — `CompanyInfo` это `name`, `legalAddress`, `vatCode`, `bankName`, `bankAccount`,
  `logoUrl` (`types/settings.ts:4-11`), и на схеме то же
  (`backend/app/modules/settings/shared/models.py:24-29`). Страна существует только внутри строки
  адреса, поэтому часовой пояс — новое поле, а не сосед существующего; заводить ли страну отдельным
  полем, не решено.
