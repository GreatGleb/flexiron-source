...
- **У компании есть страна, и хранится она кодом (П66, решено 2026-09-10).** Поле своё, а не часть
  ...
  **Выбор обязан быть с поиском, и поиск идёт по названиям на всех языках сразу** — русский
  ...
  Остаётся разнописью и владельцем не решалось: у поставщика `country` — свободная строка
  (`types/supplier.ts:21`).
  В моке у неё рядом стоят `'UK'` и `'Estonia'` — код и название в одной колонке, ровно та
  разнопись, ради ухода от которой поле у клиента и стало ссылкой.
