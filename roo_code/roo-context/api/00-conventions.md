...
- **Просмотр в журнал не пишется.** Строки про след у чтения отчёта порождены аудитом, а не
  требованием: §4.4 ТЗ перечисляет только действия, меняющие данные, и мотив исчез после П5/П17 —
  сервер вырезает недоступные числа, увидеть чужое уже нельзя.
