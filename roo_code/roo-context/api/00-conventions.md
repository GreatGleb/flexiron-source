...
**Куст «транзакционность» закрыт 2026-09-09.** Восемнадцать строк графы «Транзакционность и
идемпотентность», заданных в тринадцати доменах: шесть сняты прежними решениями (§11, П29, П31),
одна оказалась работой, а не решением, пять вопросов заданы владельцу и отвечены — П43–П48.
Обязанности назначены в §15 (атомарность, заморозка кнопки), §11 (срок и область ключа),
§22 (удаление и архив), §23 (отметка времени отчёта) и в доменном файле `bcc`. Нерешёнными в
кусте остались три остатка — атомарность карточки клиента, атомарность финансовой записи и
распознавание повторной загрузки того же файла.
