...
(выбор услуги для заказа — шестой потребитель, читающий тот же `GET /api/services`).
...
схема же даёт UUID (`UUIDMixin`, `backend/app/core/base.py:18-22`). Расхождение — известный
...
проверяется ничто** — мок не смотрит на `name` вовсе (`mocks/services.ts:95-130`), единственная
...
`grep -rln "servicesService" frontend_vue/src/views frontend_vue/src/composables` → три файла
(`useServices.ts`, `useServiceCard.ts`, `AddOrderServicesModal.vue`), и каждый из пяти экспортов
...
4. **осталось** — какое право нужно, чтобы видеть себестоимость прайс-листа и чтобы его править:
   сегодня её видит каждый, у кого включён флаг `adminServices` (`ServicesPage.vue:274`,
   `ServiceCardPage.vue:131`), тогда как в заказах та же величина закрыта `seeCost`.
