...
- **снято 2026-09-10 (§13):** `1`. Правило записано в §13 формулой `Math.max(1, ceil(total /
  pageSize))`, и девять реализаций, отдающих `0`, — расхождение с ним, а не второй вариант
  (БАГ-08). Пустая выборка это одна пустая страница, а не отсутствие страниц: клиент, увидевший
  `0`, показал бы «страница 1 из 0»;
- **снято 2026-09-10 (П20):** верхнюю границу назначает **код клиента**, а не сервер — пагинация
  принадлежит коду, сервер исполняет присланный `pageSize` и только. Сегодня границы не ставит
  никто — ни клиент (`useOrderCreate.ts:135` шлёт своё значение из пагинации), ни мок
  (`mocks/index.ts:506` знает только дефолт); свести копии к одному источнику — работа по коду.
  Общее
...
Пробелы аудита:
- **снято 2026-09-10 (§13):** в **списке** журнала и истории быть не должно — список отдаёт
  облегчённую строку, а журнал и история это содержимое карточки (та же причина, по которой у
  товаров заведён отдельный лёгкий эндпоинт справочника). В **карточке** поле остаётся, а второй
  запрос `GET /api/clients/:id/audit` (`useClientCard.ts:217-226`) — дублирование, которое
  разрешается в пользу одного источника: журнал приходит с карточкой, отдельный запрос нужен
  только для доливки страниц;
- **снято 2026-09-10 (§13, то же правило):** `interactionHistory` приходит с карточкой и не
  приходит со списком; правка идёт своими эндпоинтами, и после неё карточка перечитывается —
  иначе экран расходится с базой (тот же класс, что `load()` после ошибки, П49);
...
- закрыто и подтверждено владельцем (**П66, П67**): **`country` сервер обязан проверять по
  ...
  в интерфейсе обязан быть с поиском по названиям **на всех языках** и по коду; сегодня страну
  рисует `CustomSelect` без поля ввода (`views/admin/clients/ClientCardPage.vue:366-370`).
...
- **осталось:** `Idempotency-Key` не шлётся, и ветка мока идёт мимо `withIdempotency`
  ...
  необратимым `POST` в смысле соглашений §11 — **снято 2026-09-09**: не считается, создание
  клиента обратимо, а от двойной отправки защищает заморозка кнопки до ответа сервера
  (П47, [§15](00-conventions.md));
- **решено 2026-09-11 (П74):** поля нет — `rejectionReason` убирается из типа, из тела создания и
  из посева, а причина отказа записывается в **историю взаимодействий**: записью со своей датой и
  автором, которая не затирается следующим отказом. Сегодня поле уходит пустой строкой при типе
  `string | null`
...
- **решено 2026-09-08 (П37):** мета-записи об удалении не будет — удалять вправе владелец, и
  след лишил бы это право смысла. Признак `sensitive` при этом заводится (П5, [§9](00-conventions.md)), так что
  «удалили запись про себестоимость» станет отличимо, но фиксировать сам факт удаления сервер не
  обязан. Наполнение лога — по П36: пишется любое изменение любого свойства, а сегодня не пишется
...
- закрыто: ни `Idempotency-Key`, ни версия сущности не шлются
  ...
  применяться одной транзакцией — **решено 2026-09-09 (П49)**: не обязана. Save остаётся чередой
  `1 + N + M` запросов, частичное сохранение возможно, и контракт называет это прямо
  ([§15](00-conventions.md)). Дефектом от этого быть не перестаёт другое: `load()` после ошибки не
...
условий оплаты никого не извещают — **и решено 2026-09-09 (П51), что так и останется**: в перечень
нужных типов эти три повода не вошли, след у них журнальный (П36). Общее правило «событие — это
переход, повтор не пишет второй записи» — соглашения §10; выросший до пятнадцати перечень —
[§10.1](00-conventions.md).
...
(1) *собственная страна арендатора* — от неё зависит `local`/`export` у заказа клиента, а хранилась
она нигде (см. графу 1); **решено 2026-09-10 (П66): у компании появляется своё поле страны**, код
ISO из закрытого списка, рядом с часовым поясом ([§14](00-conventions.md)) — тем самым источник
для `local`/`export` наконец есть;
