...
Аудит, из которого собран этот файл: [`../../plans/api/audit/clients.md`](../../plans/api/audit/clients.md).
Находки про код — [`../../plans/bugs/contract-sync-clients-bugs.md`](../../plans/bugs/contract-sync-clients-bugs.md)
(семь, БАГ-01…БАГ-07). Открытые вопросы владельцу — двенадцать строк в
[`../../plans/api/audit/00-решения-владельца.md`](../../plans/api/audit/00-решения-владельца.md),
раздел `clients`.
...
- закрыто: **что вообще может прийти в теле.** Единственный редактор правит десять полей —
  ...
  `email`, `status` (`v-model` в `ClientCardPage.vue:300`, `:309`, `:318`, `:328`, `:341`, `:358`,
  `:367`, `:375`, `:405`), то есть `ClientFormData` без `rejectionReason`. Сервер обязан принимать
...
Пробелы аудита:
- **осталось:** права на удаление записи журнала не проверяются нигде — ни в моке
  (`mocks/clients.ts:1138-1144` не смотрит на пользователя), ни во фронте, при том что стирать
  историю сильнее, чем править клиента, и достижимо это из двух мест (карточка и лента). Строка
  владельцу, графа «Права»;
...
`CompanyInfo` пять полей и страны среди них нет (`types/settings.ts:4-11`),
...
четырнадцать полей без `fieldValues` (`types/client.ts:15-49`). Библиотека определений домена
...
[`../03-api-contract.md`](../03-api-contract.md) нет:
...
([`../../plans/api/contract-sync-plan.md:195`](../../plans/api/contract-sync-plan.md) —
...
([`../../plans/api/contract-sync-plan.md:259`](../../plans/api/contract-sync-plan.md)), одна из
