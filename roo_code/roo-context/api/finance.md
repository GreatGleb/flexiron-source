...
(`finance/shared/models.py:75-79`). Кто удаляет неприкреплённый файл — **решено (П31)**: уборщик
черновиков, §16 соглашений. Что обязано быть атомарным между аплоадом и Save — **решено
2026-09-09 (П49): ничего.** Связка остаётся как есть, файл уходит отдельным запросом до Save, и
дыру закрывает уборщик, а не транзакция; сам `PATCH` записи атомарен и так (§15 соглашений).
...
| **решено 2026-09-09 (П51)** · не обязан: ни закрытие счёта клиента, ни смена статуса исходящего платежа помимо просрочки в перечень нужных типов не вошли; повтор после перезапуска сервер отличает отметкой «уже уведомили» в базе (П56) | графа 2; [§10.1](00-conventions.md) |
...
| **решено 2026-09-10 наполовину** · валюта берётся из настроек при создании, дальше запись хранит свою (П19); срока жизни у записи и документа нет — не удаляем, хранение станет предметом тарифа (П61). **Осталось:** кто владеет справочником статусов платежа и перечнем типов документа архива | графа 5; [§25](00-conventions.md) |
| **снято 2026-09-10 (§4)** · из токена, и только из него — одно правило на все домены; клиенту остаётся дописать заголовок авторизации, которого он не шлёт (БАГ-01) | графа 6; [§4](00-conventions.md) |
...
| **решено 2026-09-09 (П31, П49)** · неприкреплённый файл убирает уборщик черновиков ([§16](00-conventions.md)); аплоад и Save одной транзакцией **не связываются** — остаётся как сегодня, отдельный `POST /api/uploads` до нажатия Save, а перечень атомарного закрыт заказом и складом | графа 8; [§15](00-conventions.md) |
| **решено 2026-09-11 (П68)** · `document_count` — колонка удаляется, число считается при чтении: данные свои, а хранимое устаревает молча. Кто наполняет архив документов — к производным не относится и остаётся строкой домена | графа 9; [§17](00-conventions.md) |
