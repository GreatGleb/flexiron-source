...
3. **Смена адреса — это второй HTTP-запрос без общей транзакции.** Save шлёт один PATCH и следом,
   ...
   есть партия остаётся с новым адресом и без записи о переносе. Что обязано быть атомарным —
   **решение владельца**.
...
Созданный кусок получает `status: 'available'` (`:934`) и id вида `offcut-NNN` (`:912`) — против
...
**Константами во фронте стоят, и владельца у них нет:** размер страницы `25` — шесть экземпляров,
пятью `usePagination(25)` (`useWarehouse.ts:172-176`) и дефолтами пяти веток мока
(`frontend_vue/src/services/mocks/index.ts:619`, `:660`, `:705`, `:757`, `:781`); перечень размеров
`10/25/50/100` — константа компонента (`views/admin/warehouse/WarehousePage.vue:174-179`); единица по
умолчанию — `'uom-kg'` в трёх местах (`useWarehouseBatch.ts:117`,
`useWarehouseBatchCreate.ts:40`, `useWarehouseStockCard.ts:52`) и `'uom-pcs'` ещё в трёх
(`useWarehouseOffcutCreate.ts:48`, `:229`, и в моке ручного создания нехватки
`frontend_vue/src/services/mocks/warehouse.ts:1746`); ширина реза `3` мм
...
**Что обязано быть атомарным — нигде** (решение владельца); это самая частая строка во всём файле
решений (§15 соглашений).
...
| **осталось** · кому принадлежат размер страницы, ширина реза по умолчанию и единица измерения по умолчанию | графа 1; решение владельца |
| **осталось** · остаётся ли базовая валюта склада выведенной из настроек или это отдельная настройка | графа 1; решение владельца |
| **осталось** · обязан ли склад сообщать о списании, о резке, о просроченной партии и об исчерпании остатка | графа 2; решение владельца |
| **осталось** · кто и в какой момент пишет пять складских журналов, кто автор записи и где хранятся четыре из пяти | графа 3; решение владельца |
| **осталось** · кто владеет перечнем единиц, пригодных для склада | графа 5, пункт 1; решение владельца (БАГ-15) |
| **осталось** · зачем на схеме партии колонка `exchange_rate`, если курса конвертации нет нигде | графа 5, пункт 3; решение владельца |
| **осталось** · связаны ли карта склада и адрес хранения партии, и кто владеет правилами пересчёта закупочной единицы в складскую | графа 5, пункты 2 и 4; решение владельца |
| **осталось** · какое право нужно на создание партии, на резку, на движение, на удаление записи журнала, и видит ли себестоимость партии кладовщик | графа 7; решение владельца |
| **осталось** · что обязано быть атомарным, когда клиент рвёт операцию на два-три запроса | графа 8; решение владельца |
