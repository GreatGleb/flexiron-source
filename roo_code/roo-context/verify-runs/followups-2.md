...
## 9. Регресс починен вперёд — 2026-08-28

Отклонение пункта 2 (раздел 8) в силе: бэкенд не тронут, восьмой триггер
`reserve_expiring` порождать нечем, пункт остаётся ОТКРЫТЫМ и ждёт решений человека.

Но откат отклонённой попытки снял вместе с ней и починку регресса, который та же
приёмка независимо перепроверила и назвала честной. На ветке оставался `f5c999c`
(семь эмиттеров) без глушения на время сборки сида, из-за чего один только импорт
мок-слоя писал в ленту три записи с меткой «сейчас» и давал +3 на колокольчик при
каждой загрузке страницы.

`git revert 3a7798b` конфликтует, поэтому предмет `0a2cb06` перенесён вперёд
через `git cherry-pick -n`: `notifications.ts` и `orders.ts` слились чисто,
конфликт был только в строке импорта спеки (сведён объединением: `vi` от HEAD
плюс `mockResetNotifications` от переносимого коммита) и в этом журнале.

Ни одной строки сверх предмета `0a2cb06` не добавлено. Требования пункта 2 этим
не закрываются и закрытыми не объявляются.
