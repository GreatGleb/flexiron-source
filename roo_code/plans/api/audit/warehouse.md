...
- Коды ошибок: два — `OFFCUT_NOT_FOUND` (`services/mocks/warehouse.ts:1113`) и `OFFCUT_LINKED_TO_ORDER` (`:1114`). Второй читается **только из `message`**: `if (e instanceof Error && e.message === 'OFFCUT_LINKED_TO_ORDER')` (`src/composables/useWarehouseOffcutCard.ts:385`) — против настоящего API код лежит в `ApiRequestError.code` (`src/types/api.ts:26-31`), и парная проверка у партии смотрит на оба поля (`src/composables/useWarehouseBatch.ts:341`) → находка 18.
...
18. `OFFCUT_LINKED_TO_ORDER` читается только из `e.message` (`frontend_vue/src/composables/useWarehouseOffcutCard.ts:385`), тогда как парная проверка у партии смотрит и на `code` (`frontend_vue/src/composables/useWarehouseBatch.ts:341`), а против настоящего API код лежит именно там (`frontend_vue/src/types/api.ts:26-31`).
