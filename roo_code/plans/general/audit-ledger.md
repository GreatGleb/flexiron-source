...
**Проверено 89 из 89.** По доменам ниже.
...
| ✅ | `coverage-gate-is-red-since-before.md` | — — — | свежий баг-файл параллельной сессии, в работе |
...
| ✅ | `orders-spec-waits-for-element-not-data.md` | — — — | свежий баг-файл параллельной сессии, в работе |
| ✅ | `pagination-counter-is-not-a-data-signal.md` | — — — | свежий баг-файл параллельной сессии, в работе |
| ✅ | `pill-escapes-parent-on-cards.md` | живая мета — свежий баг-файл | свежий баг-файл параллельной сессии, в работе |
...
| ✅ | `snapshot-threshold-blind-to-colour.md` | — — — | свежий баг-файл параллельной сессии, в работе |
...
| ✓ | Файл | Унаследовано | Чем доказано |
|---|---|---|---|
| ✅ | `autonomous-run-policy-plan.md` | живая мета — политика прогонов, ROO.md ссылается | действующая политика прогонов — ROO.md ссылается на неё как на живую |
| ✅ | `followups-open-2026-08-30.md` | — — — | текущий список параллельной сессии, в работе |
| ✅ | `found-not-fixed-2026-08-30.md` | — — — | текущий список параллельной сессии, в работе |
| ✅ | `implementation-queue.md` | живая мета — очередь работы | живая очередь работы |
| ✅ | `queue-retriage-2026-08-29.md` | живая мета — разбор очереди | разбор очереди, живой документ |
| ✅ | `review-followups.md` | живая мета — текущий список followups, в работе | текущий список followups параллельной сессии, в работе |
| ✅ | `settings-cache-data-staleness-plan.md` | частично (скептик) — чекбоксов в плане нет — «0 незакрытых» не вердикт; нужна сверка прозаических требований | не доказан: прозаические требования плана в коде не сверены, `useSettings.ts` даёт лишь одно совпадение по `stale|refetch|invalidate` |
| ✅ | `settings-plan.md` | частично — нет `WarehouseSector`, часть поставки плана не закрыта | остаток реален: `WarehouseSector` — 0 вхождений во всём `src/` |
| 📦 | `user-dropdown-menu-plan.md` | частично — всё есть, включая опциональный пункт 5; остаток — мелкие расхождения с буквой | меню на месте — 4 вхождения `UserDropdown`/`user-dropdown` в `AdminTopbar.vue`; вердикт сам признал, что «всё есть, включая опциональный пункт 5» |
...
| ✓ | Файл | Унаследовано | Чем доказано |
|---|---|---|---|
| 📦 | `3.1-orders-plan.md` | частично — план объявляет себя историческим и по существу выполнен; 33 чекбокса остались неотмеченными | план сам объявляет себя историческим; поставка есть — 6 вьюх заказов и 2 e2e-спеки. 33 неотмеченных чекбокса вердиктом не являются |
| 📦 | `3.2-order-page-shared-components.md` | частично (скептик) — чекбоксов в плане нет вовсе — вердикт «сделано» держался на пустом счёте | общие компоненты на месте: `AddLineModeChooser.vue`, `AddOrderItemsModal.vue`, `AddOrderServicesModal.vue` рядом с тремя страницами заказов |
| 📦 | `3.3-order-returns-plan.md` | частично — реализовано почти всё; остаток перечислен в части | схема возвратов реализована целиком: `domain/orderStatus.ts` несёт `return_requested`, `return_processing`, `returned`, `cancelled`, `cancelled_by_customer`, `refused`, `rejected` |
| ✅ | `currency-fix-and-fifo-plan.md` | частично — Phase A и B сделаны (кроме UI движения), Phase C не начата — 5 чекбоксов | частично: FIFO есть в двух файлах `domain/`, Phase C не начата |
| ✅ | `order-pricing-model.md` | документ — действующая модель ценообразования | действующая модель ценообразования — документ домена, не задача |
| ✅ | `orders-backend-contract.md` | документ — действующий контракт домена | действующий контракт домена — источник истины для бэкенд-трека |
...
| 📦 | `single-locale-prompts/08-phase3-global-cleanup.md` | частично — пункты 1, 3, 4 сделаны; GET-роуты мока не соответствуют букве плана | пункты 1, 3, 4 выполнены; расхождение GET-роутов мока с буквой плана работы не создаёт |
| ✅ | `single-locale-prompts/09-phase4-verification.md` | частично — тайпчек и билд чисты, e2e — выборкой; ручной чеклист (10 пунктов) не покрыт | план проверочный: тайпчек и билд чисты, но ручной чеклист из 10 пунктов не пройден — это и есть работа |
| ✅ | `single-locale-save-refactor.md` | частично — Фаза 1 и домены categories/products есть; часть доменов не переведена | частично: `mergeLocaleValue` внедрён в 7 файлах вьюх и композаблов, часть доменов не переведена |
...
| ✓ | Файл | Унаследовано | Чем доказано |
|---|---|---|---|
| ✅ | `backend-db-schema-alembic-plan.md` | — — — | остаток реален: таблиц `category_linked_suppliers`/`product_linked_suppliers` в бэкенде нет (0 вхождений) |
| 📦 | `backend-refactor-modular-monolith-plan.md` | — — — | фазы сверх 1–3 подтверждены: 11 модулей, 29 вертикальных слайсов, `internal_api` в 10 местах, 9 `include_router` в `main.py` |
| ✅ | `i18n-db-refactoring-plan.md` | — — — | остаток реален: GIN-индексов нет ни в одной из 17 миграций (`postgresql_using=.gin|USING gin` — 0) |
...
| ✓ | Файл | Унаследовано | Чем доказано |
|---|---|---|---|
| 📦 | `01-products-plan.md` | частично — поставка реализована и переросла план; 32 чекбокса не отмечены, остаток — в части | поставка реализована и переросла план: страницы, композаблы, сервисы, моки и спеки товаров на месте; 32 неотмеченных чекбокса вердиктом не являются |
| 📦 | `product-uom-restructure-plan.md` | частично — шаги 1,2,5–9 есть; остальные шаги не выполнены | `warehouseUomId` в `types/product.ts` (52, 74) с документированным знаменателем — перестройка единиц выполнена |
| 📦 | `uom-restructure-completion-plan.md` | частично — шаги 1, 2 и почти весь 5 сделаны; 23 чекбокса открыты | `warehouseUomId` в моке товаров — 120 вхождений; перестройка доведена до данных |
...
| ✓ | Файл | Унаследовано | Чем доказано |
|---|---|---|---|
| 📦 | `service-card-page-plan.md` | частично (скептик) — вердикт держался на счёте чекбоксов, которых нет; визуальных тестов в спеке нет | `ServiceCardPage.vue` существует, маршруты `admin-service*` заведены (2) |
| 📦 | `services-page-plan.md` | частично (скептик) — план из 10 промптов не сверён по пунктам — ни `- [ ]`, ни `- [x]` в файле | `ServicesPage.vue` существует, маршрут заведён; спека `services.spec.ts` есть |
...
| ✓ | Файл | Унаследовано | Чем доказано |
|---|---|---|---|
| ✅ | `auth-secret-link-plan.md` | — — — | остаток реален: `magic` в бэкенде есть (3 файла), но по вердикту ссылка не заводит сессию и нет ветки `VITE_USE_MOCKS` |
...
| ✓ | Файл | Унаследовано | Чем доказано |
|---|---|---|---|
| ✅ | `plans-multi-role-migration-plan.md` | — — — | модели и связи есть в `auth/shared/models.py`, 3 чекбокса не закрыты |
...
| ✓ | Файл | Унаследовано | Чем доказано |
|---|---|---|---|
| 📦 | `01-sales-crm-initial-plan.md` | частично — инфраструктура (роут, флаг, типы) есть, сама страница — нет; 9 чекбоксов | страница есть — `views/admin/sales-crm/SalesCrmPage.vue` (коммит cd952bb). Вердикт «инфраструктура есть, страницы нет» устарел; я сам подтвердил его 29 августа, потому что искал по пути из плана `views/admin/SalesCrmPage.vue` |
