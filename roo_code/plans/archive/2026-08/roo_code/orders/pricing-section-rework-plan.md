...
> **Состояние:** раздел 1 (карточка товара) актуален. Разделы 2 и 3 (карточка
> заказа и формулы) **устарели** и заменены переработкой ценообразования заказа —
> подробности в пометках к разделам.
...
## 2. Карточка заказа — реструктуризация секции цен ⛔ УСТАРЕЛО

> **Этот раздел не выполнять.** Он описывает расчёт, который заменён целиком:
> себестоимость как редактируемое поле заказа, маржа и НДС процентами на заказе,
> и цена клиента как `себестоимость × (1 + ндс) × (1 + маржа) × (1 − скидка)`.
> Так НДС начислялся на себестоимость, а маржа — поверх НДС; на ORD-001 это давало
> 26 438,50 вместо 22 990,00.
>
> Действующая модель — [order-pricing-model.md](../../../../orders/order-pricing-model.md): источник
...
## 3. Формулы расчёта ⛔ УСТАРЕЛО

> Формула ниже неверна — см. пометку к разделу 2.
...
> Строки про заказ здесь **устарели** вместе с разделом 2: `marginPercent` на
> заказе и `costPrice` в форме не появились и не появятся — маржа и скидка живут
> на строке. Актуальны только две строки про карточку товара.
