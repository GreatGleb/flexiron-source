...
**Разблокировано — П69.** [`../api/audit/00-решения-владельца.md`](../api/audit/00-решения-владельца.md), строка 1952:

> **решено 2026-09-11:** из **отгруженного минус возвращённое** — по факту движения товара;
> заказанное и выставленное выручкой не считаются, и три экрана обязаны сходиться → П69

Работа по коду: `salesMtd` перестаёт считать заказанное; правило «отгруженное минус возвращённое»
берётся из единственного места, где оно уже написано в том же файле, и все три экрана считают
одним предикатом.
...
**Разблокировано — П63.** [`../api/audit/00-решения-владельца.md`](../api/audit/00-решения-владельца.md), строка 1944:

> **решено 2026-09-10:** перечень «ожидающих» статусов остаётся **в коде** и фиксирован, новые
> статусы настроек в сводку не попадают — но фронт обязан показать подсказку, какие именно статусы
> считаются ожидающими (П63); границу месяца сервер режет по часовому поясу компании (П62)

Работа по коду прежняя и теперь разблокирована: предикат `isPendingOrder(status)` в
`frontend_vue/src/domain/orderStatus.ts`, рядом с двумя существующими, и вызов его из мока. Плюс
подсказка на экране сводки — какие именно статусы в число попали.
