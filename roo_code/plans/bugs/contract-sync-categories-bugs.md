...
## БАГ-09 — селект родителя в модале создания собран из текущей страницы списка

**File:** `frontend_vue/src/views/admin/products/CategoriesPage.vue:52-55,225`, `frontend_vue/src/composables/useCategories.ts:26-28`
**Severity:** Medium — родителем можно назначить только категорию, видимую прямо сейчас; при активном поиске выбор сужается до найденных.
**Источник:** К4 (форма запроса), Contract

### Problem

`parentOptions` строится напрямую из `items` — того же массива, что рисует таблицу:
...
`items` — это **одна страница** выдачи: `useCategories.load()` кладёт туда `res.items`
(`frontend_vue/src/composables/useCategories.ts:27`), а страница и её размер приходят из
`usePagination(25)` (`:18`). Значит на второй странице списка в селекте родителя
(`CategoriesPage.vue:225`) окажутся только категории второй страницы, а при непустом
...
Брать справочник родителей отдельным запросом, как это делает карточка товара
(`getCategories({ search: '' }, 1, 999)`, `frontend_vue/src/composables/useProductCard.ts:132`),
и не переиспользовать под него отфильтрованную выдачу таблицы. Форма справочного ответа —
решение владельца, строка уже стоит в
`roo_code/plans/api/audit/00-решения-владельца.md` (графа «Форма ответа для справочников»).

### Future rule

Список, который рисует таблицу, и список, который наполняет выбор связи, — разные списки.
Первый пагинируется и фильтруется, второй обязан быть полным; переиспользование первого во
втором молча сужает выбор ровно тогда, когда данных стало много.
...
| | Contract | `CategoriesPage.vue` | БАГ-09: селект родителя в модале создания собран из текущей страницы списка |
