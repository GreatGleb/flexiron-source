...
**ЖДЁТ ВЛАДЕЛЬЦА — и будет ждать: строки под эту находку в файле решений нет вовсе.**
Перепроверено 2026-09-12: ни одна из семнадцати строк `- analytics · ` в [`../api/audit/00-решения-владельца.md`](../api/audit/00-решения-владельца.md)
не задаёт вопрос «ключ маршрута против ключа перевода». Закрытие всех 189 строк файла решений
(коммит `9410d9d`, «все 189 строк закрыты») этой находки **не касается** — её вопроса среди
этих 189 нет.

Развилка прежняя, и выбрать её должен владелец, потому что это про контракт, а не про опечатку:
...
**Чего не хватает, дословно:** одной строки в [`../api/audit/00-решения-владельца.md`](../api/audit/00-решения-владельца.md)
вида `- analytics · Формы запроса и ответа · чем сервер подписывает секцию аналитики — тем же
ключом, что и маршрут, или отдельным полем · …`. Либо строка заводится, либо вопрос признаётся
инженерным и находка чинится без владельца.
...
**Разблокировано.** [`../api/audit/00-решения-владельца.md`](../api/audit/00-решения-владельца.md), строка 1739:

> **снято 2026-09-10 (§17, тот же приём, что у `entityRouteName`):** иконка — представление, её
> выбирает клиент по ключу метрики, а сервер её не назначает и в ответе не несёт

Номера П у этого вердикта нет: он вынесен по §17 «Производные значения» общих соглашений
([`00-conventions.md`](../../roo-context/api/00-conventions.md)), прецедентом `entityRouteName` (там же, раздел 17).

Работа по коду, которую вердикт назначил: `icon` уходит из `AnalyticsSectionPreview`/`KpiItem` и
из ответов мока; карта «ключ метрики → иконка» заводится одна на восемь страниц аналитики, вместо
пяти знакомых имён у дашборда и жёстких путей у остальных семи.
...
**Разблокировано — П70.** [`../api/audit/00-решения-владельца.md`](../api/audit/00-решения-владельца.md), строка 1719:

> **решено 2026-09-11:** всегда доля от целого, сумма по таблице равна 100; ширину полосы фронт
> считает сам, деля на максимум → П70

Работа по коду: `salesByCategory` (сумма 203) и `topClients` (сумма 244) в
`frontend_vue/src/services/mocks/analytics.ts` приводятся к сумме 100, как уже сделано у
`refusalReasons`; `width: item.percentage + '%'` в шести шаблонах заменяется на долю от максимума
ряда, иначе полоса «100 %» перестанет доходить до края.
...
**Разблокировано.** [`../api/audit/00-решения-владельца.md`](../api/audit/00-решения-владельца.md), строка 1737:

> **снято 2026-09-10 (§13):** ничем не отвечает — сервер вправе зажать страницу в границы и
> вернуть принятое значение вместе с пустым `items`; «страницы нет» отдельным состоянием не
> является, и кода под него не нужно

Номера П у этого вердикта нет: он вынесен по §13 «Пагинация и списки» общих соглашений
([`00-conventions.md`](../../roo-context/api/00-conventions.md)); само правило «пагинация принадлежит коду» там помечено П20.

Работа по коду: кода ошибки заводить не нужно — нужно, чтобы мок и сервер вели себя одинаково,
то есть зажимали значение в границы и возвращали принятое вместе с пустым `items`, а не
принимали любой хвост включая слэши (`frontend_vue/src/services/mocks/index.ts:310`).
...
**Разблокировано вместе с БАГ-07.** [`../api/audit/00-решения-владельца.md`](../api/audit/00-решения-владельца.md), строка 1737 (§13):
каталог кодов домену не нужен — «страницы нет» отдельным состоянием не является. Значит ждать
больше нечего, и оформление ошибки привязывается не к коду домена, а к общему каталогу §2
([`00-conventions.md`](../../roo-context/api/00-conventions.md)).

Номера П у этого вердикта нет: он вынесен §13 и §2 соглашений.

Работа по коду: сырой текст сервера заменяется на сообщение из словаря по коду общего каталога;
домен своих кодов не заводит.
...
## БАГ-10 — `AlertItem` без идентификатора и без ссылки на сущность: по тревоге некуда перейти

**File:** `frontend_vue/src/types/analytics.ts:31-37`, отрисовка — `frontend_vue/src/views/admin/analytics/DashboardPage.vue:152-162`
**Severity:** Medium — блок «что горит» показывает текст, с которым нельзя ничего сделать; ключ списка неуникален по построению
**Источник:** решение владельца П55 (§10.4 соглашений)

### Problem

Тип несёт три поля и ни одного адреса:
...
Отсюда два следствия, оба видны на дашборде.

**Перейти некуда.** Строка тревоги — обычный `<tr>` без ссылки и без обработчика (`DashboardPage.vue:152-162`),
хотя каждая тревога описывает конкретную сущность: «Запас листа 2 мм критически мал», «Поставка
...
одинаковый ключ, и Vue переиспользует узел не по назначению. Идентификатора, которым это чинится, у
`AlertItem` нет.

Владелец назвал решение: **П55 (§10.4 соглашений) — тревоги дашборда есть проекция ленты
уведомлений, а не свой механизм.** Там же прямо: «сегодняшний `AlertItem` это `type`, `description`
и `status` без идентификатора и без ссылки на сущность (`types/analytics.ts:31-37`), то есть по
тревоге некуда перейти; как проекция уведомления она обязана нести то же, что несёт строка ленты».

### Fix

TBD — по П55 тревога перестаёт быть отдельной сущностью: сервер помечает уведомления флагом «требует
действия сейчас», а блок `alerts` показывает помеченные. Тогда `AlertItem` либо исчезает в пользу
`Notification`, либо получает те же `id`, `entityType`, `entityId`, `entityRouteName`.

### Future rule

Строка, которая сообщает о проблеме, обязана вести к месту, где её решают. Тип, в котором нет
адреса сущности, обрекает интерфейс на тупик и заодно лишает список честного `:key`.
...
| | Contract | `types/analytics.ts` | БАГ-10: `AlertItem` без идентификатора и без ссылки на сущность — по тревоге некуда перейти |
