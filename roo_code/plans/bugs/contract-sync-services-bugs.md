...
`price_unit` удалён, добавлены `currency_id` → `currencies.id` и `uom_id` → `uoms.id`, обе
`nullable`, `ondelete="RESTRICT"`, под индексом — как `products.currency_id`/`sale_uom_id`.

- модель: `backend/app/modules/services/shared/models.py:31-36`, `:38-43`;
  объявления колонки нет: `grep -c "price_unit.*mapped_column"` по файлу → 0 (единственное
  оставшееся вхождение слова — строка-комментарий `:29`, объясняющая, что здесь было);
- ревизия: `backend/alembic/versions/7fff8d1e5810_phase_16_service_currency_uom.py`;
- перед ней потребовалась ревизия слияния — граф разошёлся на две головы ещё в июне, и
  `alembic upgrade head` падал, не применив ничего. Отдельная находка:
  [`alembic-graph-had-two-heads.md`](alembic-graph-had-two-heads.md).
...
`get_uom_by_code` (`backend/app/modules/settings/features/crud/repository.py:174-190`), а не
...
`asyncpg` поставлен `pip install --target`):

| Проверка | Результат |
|---|---|
| `alembic upgrade head` с нуля | все 19 ревизий, exit 0 |
| `\d services` | `price_unit` нет; два FK с `ON DELETE RESTRICT`, два индекса |
| `alembic check` на услуги | расхождений по `services` ноль (по девяти чужим таблицам — есть, это БАГ-02 соседнего файла) |
| перенос на данных | два арендатора, у обоих есть `EUR` и `vnt`: каждая услуга получила ссылки **своего** арендатора (`aaaa…` против `bbbb…`) |
| `'USD/h'` | сматчилось по английской локали единицы |
| `'XXX/zzz'` и арендатор без справочников | остались `NULL`, ничего не уронили |
| `alembic downgrade -1` | `price_unit` вернулся; `'EUR/vnt'` → `'EUR/pcs'` — обратный проход лоссовый, и это записано в самой ревизии |
| `python3 -m unittest discover -s tests -t .` | 14 тестов, OK |
| `from app.main import app` | импортируется |

**Подтверждение (не автор правки, 2026-09-07).** Миграция прогнана заново на своей базе
(`flexiron_close` в том же контейнере на 5433), а не принята по отчёту:

| Что проверял сам | Что вернулось |
|---|---|
| `alembic upgrade head` с нуля | 19 ревизий, exit 0 |
| `\d services` | `price_unit` нет; `ix_services_currency_id` и `ix_services_uom_id` есть, оба FK с `ON DELETE RESTRICT` |
| перенос: `'EUR/vnt'` у двух арендаторов | каждая услуга получила валюту и единицу **своего** арендатора (`aaaa…` и `bbbb…`), пересечения нет |
| `'USD/h'` | сматчилось единицей, у которой заполнена только английская локаль |
| `'XXX/zzz'` | обе ссылки `NULL`, ничего не упало |
| `'USD/vnt'` у арендатора без `USD` | валюта `NULL`, единица сматчилась — частичное разрешение, как и задумано под `nullable` |
| `alembic downgrade -1` | `price_unit` вернулся, `'EUR/vnt'` → `'EUR/pcs'` — лоссовость подтверждена замером |
| `python3 -m unittest discover -s tests -t .` | 14 тестов, OK |

Отдельно проверено, что `index=True` внутри `op.add_column` не проглатывается: индексы в
`\d services` есть, то есть alembic их создаёт (в проекте так уже сделано в
`4e8c9a3f1b2d_add_secret_link_token_to_users.py:26-35`).

Поправлена одна цифра отчёта: `grep -c "price_unit"` по модели даёт **1**, а не 0 — остаётся
строка-комментарий `:29`. Объявления колонки нет, это проверяется грепом по
`price_unit.*mapped_column` → 0.

**Что осталось незакрытым внутри этого бага:** `nullable` в колонке против обязательных
`currencyId`/`uomId` на проводе (`types/service.ts:25-30`). NOT NULL на существующих строках
не гарантируется — у арендатора может не оказаться валюты под свой же `price_unit`, — поэтому
обязательность обязан держать слайс. Записано в контракт (`api/services.md`), туда же уехало
описание всей правки вместо прежней врезки «модель осталась в мире сваренной строки».
...
**Разблокировано 2026-09-12. Было:** «Решение о поведении принимает владелец — строка вынесена в
`00-решения-владельца.md`». **Стало — П44**
(`roo_code/plans/api/audit/00-решения-владельца.md:1673`, текст решения — там же, `:736-756`):

> услуга не удаляется, а помечается архивной — из выборов пропадает, в старых заказах остаётся
> читаемой; валюту и единицу при живых ссылках сервер удалить не даёт и отвечает отказом → П44

Выбран третий из трёх перечисленных выше путей, и выбран точно: **мягкое удаление, не 409**.
Обещанный старым контрактом `SERVICE_IN_USE` не появляется — удалению услуги никто не отказывает,
потому что отказывать нечему: запись остаётся на месте с признаком архивной.
...
- отказ по живым ссылкам остаётся **правилом справочников** (единицы, валюты), а не услуг; под П44
  всё, что ссылается на справочник, обязано стать `RESTRICT`. У услуг это уже так —
  `backend/app/modules/services/shared/models.py:33,40`.

Строка «контракт обещает одно, код делает другое» закрывается не приведением кода к обещанию, а
исправлением самого обещания: `SERVICE_IN_USE` уходит из контракта.

Эта находка триажем 2026-09-09 стояла в группе В («ждёт нерешённой строки»); делта №1 перевела её
в Б по П44, а пометка в этом файле осталась прежней до 2026-09-12. Разбор —
