...
Не решение прогона: что арендатор получает после регистрации — решение владельца №1 (фичи и
тарифы). До него текст баннера либо снимается, либо остаётся с осознанием, что за ним нет
реализации. Контракт домена фиксирует расхождение — раздел «Чего в домене нет».
