...
**Разблокировано — П53 и П58.** [`../api/audit/00-решения-владельца.md`](../api/audit/00-решения-владельца.md), строка 1804:

> **снято 2026-09-10 (следует из П53 и П58):** в `/me` его быть не должно. Ссылка доставляется
> письмом или копированием в момент приглашения (П58), а `/me` — это «кто я сейчас»; отдавать
> секрет в каждом ответе о себе значит держать его там, где он не нужен

Работа по коду: `secret_link` убирается из `MeResponse`
(`backend/app/modules/auth/features/me/schemas.py:19`) и из сборки ответа (`me/domain.py:22-24`);
доставка ссылки переезжает на почту (П53) и на момент приглашения (П58). `UserInfo` во фронте не
меняется.
...
**Разблокировано.** [`../api/audit/00-решения-владельца.md`](../api/audit/00-решения-владельца.md), строка 1806:

> **снято 2026-09-10 (§1, §2):** присылает явно — `fieldErrors` объявлены в `ApiRequestError` и
> клиент их уже читает; вывод поля из подстроки сообщения — дефект, а не форма контракта

Номера П у этого вердикта нет: он вынесен по §1 «Конверт ответа» и §2 «Каталог кодов ошибок»
общих соглашений ([`00-conventions.md`](../../roo-context/api/00-conventions.md)).

Работа по коду: `inferFieldFromMessage` (`frontend_vue/src/services/api.ts:86-106`) удаляется,
клиент читает `fieldErrors`; сервер обязан класть имя поля в конверт, а не в прозу сообщения.
Обещание «контракт называет текущие сообщения дословно, чтобы бэкенд знал, что их нельзя менять
молча» снимается — привязки к тексту больше нет.
...
**Разблокировано.** [`../api/audit/00-решения-владельца.md`](../api/audit/00-решения-владельца.md), строка 1798:

> **снято 2026-09-10:** схема уже отвечает — уникальна **пара «арендатор + email»**, индекс
> `ix_users_tenant_id_email`; известное расхождение кода заведено как БАГ-13
> (`scalar_one_or_none()` по всей таблице)

Номера П у этого вердикта нет: он вынесен старшинством схемы, а не новым решением владельца —
вопрос был негодным, ответ уже стоял в миграции.

Работа по коду: искать пользователя **с арендатором** — `login/repository.py:20-23` и
`register/repository.py:40-42` перестают брать `scalar_one_or_none()` по всей таблице; колонка
`email` остаётся без `unique`, составной индекс остаётся. Ветка «снять составной индекс и
объявить колонку `unique`» закрыта.
...
## БАГ-16 — секретная ссылка собирается тремя копиями, и токен добирает чужой домен

**File:** `backend/app/modules/settings/features/profile/domain.py:22-41`
**Severity:** Medium — колонка `users.secret_link_token` принадлежит модулю auth, а пишет в неё модуль settings; URL той же ссылки склеен в трёх местах.
**Источник:** аудит домена 2026-09-04, линза К7 (аудит называл четыре места употребления `secret_link_token`, их четырнадцать)

### Problem

`_ensure_secret_link` (`backend/app/modules/settings/features/profile/domain.py:22-41`) сама
**генерирует** токен, если его нет (`:32` — `secrets.token_urlsafe(48)`), и пишет его через
`update_user` (`:33`). Зовут её два роута чужого домена: `GET /api/settings/profile` (`:54`) и
`PATCH /api/settings/profile` (`:99`), то есть чтение профиля меняет данные.

Тот же URL собирается тремя независимыми f-строками: `register/domain.py:154`, `me/domain.py:24`
и `settings/features/profile/domain.py:38-41`. Длина токена задана в auth именованной константой
`_SECRET_LINK_BYTES = 48` (`backend/app/modules/auth/features/register/domain.py:27`), а в копии
settings — литералом `48` (`:32`): разойдутся молча.

Следствие для контракта: утверждение «токен выдаётся один раз при регистрации и не меняется
нигде» неверно — он добирается при первом чтении профиля. Ротации при этом по-прежнему нет:
существующий токен не перезаписывается никогда.

### Fix

Сборку URL и генерацию токена держать в одном месте модуля auth (internal_api), а settings звать
её. Задача бэкенда; контракт домена фиксирует наблюдение — раздел `GET /api/auth/link`.

---

## БАГ-17 — страница регистрации обещает 14-дневный триал, которого нет ни в одной строке кода

**File:** `frontend_vue/src/views/public/RegisterPage.vue:12-26`
**Severity:** Low — обещание пользователю, за которым не стоит ни поля, ни эндпоинта; но обещание платное.
**Источник:** аудит домена 2026-09-04, линза К7 (аудит утверждал, что `grep -rni "trial"` пуст по обоим деревьям — по фронту он не пуст)

### Problem

Баннер над формой регистрации показывает `reg.trial` — «Вам будет доступно 14 дней базового
триала» (`frontend_vue/src/i18n/public.ts:82`, английский `:265`, литовский `:453`; разметка
`frontend_vue/src/views/public/RegisterPage.vue:12-26`).

В коде за этим нет ничего: `grep -rni "trial" backend/` пуст целиком, ответ регистрации о сроке не
говорит (`backend/app/modules/auth/features/register/schemas.py:29-42`), модели тарифов
(`backend/app/modules/billing/shared/models.py`) роутов не имеют ни одного, а поля «до какого числа
триал» нет ни у арендатора, ни у пользователя (`backend/app/modules/auth/shared/models.py:39-81`).

Поправка к аудиту: строка «`grep -rni "trial" backend/app frontend_vue/src` пуст» неверна — пуст
только бэкенд.

### Fix

**ЖДЁТ ВЛАДЕЛЬЦА — засеянное решение №1 не отвечено.**
Перепроверено 2026-09-12: раздел «Засеяно до прогона», пункт 1 «Фичи и тарифы: откуда админка
берёт матрицу» ([`../api/audit/00-решения-владельца.md`](../api/audit/00-решения-владельца.md), строки 1422–1437) стоит без
подраздела с вердиктом — там только «Что решить» и «Почему не решает прогон». Общие соглашения
подтверждают это прямым текстом: «Не решено этим кустом: что сервер отвечает на запрос к
**выключенной фиче** … и что сильнее, тариф или переопределение арендатора»
([`00-conventions.md`](../../roo-context/api/00-conventions.md), §7).

П13 и П14 закрыли соседнее — где живёт реестр фич и что `adminWarehouse` делится, — но не то,
**что арендатор получает после регистрации**. Замер: `grep -rn "триал\|trial\|14 дней"` по
[`../api/audit/00-решения-владельца.md`](../api/audit/00-решения-владельца.md) — ни одного попадания; П доходят до П75, тарифного среди них нет.

Поэтому вердикт прежний: текст баннера либо снимается, либо остаётся с осознанием, что за ним нет
реализации; контракт домена фиксирует расхождение в разделе «Чего в домене нет».
...
| | Backend | `backend/.../settings/profile` | БАГ-16: секретная ссылка собирается тремя копиями, токен добирает чужой домен |
| | Contract | `RegisterPage.vue` | БАГ-17: баннер обещает 14-дневный триал, реализации нет нигде |
