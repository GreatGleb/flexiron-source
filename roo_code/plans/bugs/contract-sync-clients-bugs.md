...
**Разблокировано.** [`../api/audit/00-решения-владельца.md`](../api/audit/00-решения-владельца.md), строка 1578:

> **снято 2026-09-10 (§19):** получает. Адресация индексом массива запрещена прямо — «клиент не
> имеет права выводить `id` из позиции», а выдаёт его сервер; удаление по индексу это дефект
> (БАГ-01), а не форма контракта
...
Работа по коду: у записи взаимодействия появляется `id`, выданный сервером; удаление адресуется
`DELETE /api/clients/:id/interactions/:entryId` по нему, а не по позиции в массиве. Сравнение по
JSON-содержимому уходит вместе с этим.
...
**Разблокировано — П74.** [`../api/audit/00-решения-владельца.md`](../api/audit/00-решения-владельца.md), строка 1580:

> **решено 2026-09-11:** поля нет — убирается из типа, из тела создания и из посева; причина
> отказа записывается в историю взаимодействий, со своей датой и автором → П74

Работа по коду: `rejectionReason` убирается из `ClientFormData`, из тела `POST /api/clients` и из
сидов мока; место в карточке ему не заводится. Причина отказа становится обычной записью истории
взаимодействий.
...
## БАГ-08 — отказ на середине сохранения не перечитывает карточку, и экран расходится с базой

**File:** `frontend_vue/src/composables/useClientCard.ts:266-320` (обработчик отказа — `:314-317`)
**Severity:** High — часть изменений уже на сервере, экран показывает своё, повтор Save шлёт удаления по сместившимся индексам
**Источник:** решение владельца П43/П49 (§15 соглашений), Л8 (Save UX)

### Problem

`save()` — это не один запрос, а `1 + N + M` последовательных: `PATCH /api/clients/:id` дельтой полей
(`:274`), затем по `DELETE` на каждое исчезнувшее взаимодействие (`:298`) и по `POST` на каждое
добавленное (`:303`). Так это и записано в соглашениях: `clients` — строка «**1 + N + M**
последовательных запросов» (`roo_code/roo-context/api/00-conventions.md`, §15).

Отказ на любом из них приводит в один и тот же обработчик:
...
`load()` отсюда не зовётся: единственный его вызов после монтирования стоит в `discard()` (`:322-325`).
Значит после падения на третьем `DELETE` из пяти:

- на сервере два взаимодействия удалены, три нет;
- `client.value` по-прежнему показывает список, в котором нет всех пяти;
- `capturedInteractions` и `dirty.capture()` не выполнялись — они стоят ПОСЛЕ цикла (`:308-312`),
  то есть снимок остался дозапросным;
- человеку показан один тост «не сохранилось», хотя часть сохранилась.

Второе нажатие Save считает дельту от устаревшего снимка и шлёт `deleteClientInteraction(id, idx)`
(`:297-298`) — удаление по ИНДЕКСУ, а индексы на сервере уже сместились двумя первыми удалениями.
То есть повтор удаляет не то, что человек удалял.

П49 (добор к П43) решил только границу транзакции: «и карточка клиента, и финансовая запись остаются
как есть» — то есть череда независимых запросов остаётся, частичное сохранение возможно. Поведение
ПОСЛЕ частичного отказа этим решением не задано, и сегодня его нет ни у одного из семи многозапросных
Save (§15 перечисляет все семь).

### Fix

TBD. Минимум — перечитать карточку после отказа (`await load()`, как это делает `discard()`), чтобы
экран показывал то, что действительно лежит в базе, и второй Save считался от настоящего состояния.
Полное решение упирается в удаление по индексу: пока `DELETE` адресует взаимодействие порядковым
номером, любой частичный отказ делает повтор опасным — нужен `id` записи (см. БАГ-01 того же файла).

### Future rule

У многозапросного Save обработчик отказа обязан сказать, в каком состоянии осталась база, а не только
что «не получилось». Тост без перечитывания оставляет экран в состоянии, которого на сервере нет ни
до операции, ни после неё. Обратная крайность — `catch { load() }` поверх несохранённого черновика —
такой же дефект (`contract-sync-finance-bugs.md`, БАГ-12); выбор между ними определяется тем, есть ли
в форме что терять.
...
| | Runtime | `useClientCard.ts` | БАГ-08: отказ на середине сохранения не перечитывает карточку — экран расходится с базой |
