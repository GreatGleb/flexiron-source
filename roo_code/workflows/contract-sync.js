...
/**
 * Черновики отклонённых попыток — вне репозитория.
 *
 * Откат правит историю, а работу сохраняет: следующая попытка правит названное приёмщиком, а не
 * пишет домен заново. Место выбрано вне дерева не из аккуратности: черновик, оставленный в
 * `roo-context/api/`, уезжал в коммит следующего домена (2026-09-04, дважды) и показывал
 * следующему автору грязное дерево, которое его инструкция велит считать чужой работой.
 */
const DRAFT_DIR = '/tmp/contract-drafts'
...
/**
 * Аудиты уже на диске — не переделывать.
 *
 * Прогон 2026-09-04 свёл все 17 аудитов (пустых граф ноль) и лёг на написании. Перезапуск без
 * этого флага потратил бы шесть часов на переделку готового, а ошибка была не в аудитах.
 * Фаза аудита пропускается ЦЕЛИКОМ: файлы читаются с диска как результат.
 */
const SKIP_AUDIT = Boolean(args && args.skipAudit)
...
function auditPrompt(domain, afterCrash, attempt, lastReason, readyDeps, missingDeps, branch) {
  return [
    ...
    '   Ссылка не просто ставится, а ПРОВЕРЯЕТСЯ резолвером — по своему файлу, целиком:',
    `     cd frontend_vue && CONTRACT_REFS=roo_code/plans/api/audit/${domain}.md \\`,
    '       npx vitest run src/services/contractRefs.spec.ts',
    '   «битых 0» по своему файлу обязательно, и вывод резолвера вставь в notes целиком:',
    '   счёт без вывода не принимается. Аудит пробы categories поставил tenant_id на миграцию',
    '   :44, где на самом деле колонка id, — ошибку поймал только следующий агент, и она',
    '   успела переехать в контракт дословно.',
    ...
    '   Ссылаться на него С НОМЕРОМ СТРОКИ ЗАПРЕЩЕНО — цитируй текстом, что он обещал.',
    '   Монолит удаляется задачей 38, и номер строки в нём станет мусором; а пока он жив,',
    '   номера всё равно врут: три отказа прогона 2026-09-04 были ровно этим.',
...
function writePrompt(domain, audit, fixReason, afterCrash, attempt, readyNeighbours, branch) {
  ...
  return [
    ...
    afterCrash
      ...
      : '',
    fixReason
      ? `\nРАБОТА НАД ОШИБКАМИ — попытка ${attempt} из ${MAX_ATTEMPTS}, а не чистый лист.\n\nТВОЙ ЧЕРНОВИК ЛЕЖИТ ЗДЕСЬ: ${DRAFT_DIR}/${domain}.md — это отклонённая версия целиком.\nНачни с того, что скопируй его на место (cp ${DRAFT_DIR}/${domain}.md roo_code/roo-context/api/${domain}.md)\nи ПРАВЬ названное. Писать домен заново запрещено: приёмщик подтвердил в прошлой версии\nбольшую часть, и переписанное придётся доказывать с нуля — так домены и сгорали по три\nпопытки. Коммит прошлой попытки откачен, файла в истории ветки нет; черновик — не история,\nа рабочая копия.\n\nРазбор приёмщика проверен, ему верить больше, чем своему первому впечатлению:\n\n${fixReason}\n\nЧини именно названное, остального не изобретай: подтверждённое приёмщиком воспроизведи как\nбыло — переписывание принятого само по себе основание для отказа.\n`
      : '',
    readyNeighbours.length
      ...
      : '',
    `ЧЕРНОВИК ПРОШЛОГО ПРОГОНА. Проверь, есть ли ${DRAFT_DIR}/${domain}.md (ls). Есть — это`,
    'отклонённая или недоделанная версия этого домена, и начинать надо с неё, а не с нуля:',
    `   cp ${DRAFT_DIR}/${domain}.md roo_code/roo-context/api/${domain}.md`,
    'Дальше проверяй её саму против кода — она НЕ принята приёмкой, доверять ей нельзя, но',
    'переписывать написанное заново значит потерять часы и доказывать всё с начала. Нет файла —',
    'пиши с чистого листа, это нормально.',
    '',
    'Требования:',
    ...
    '9. Ссылки проверяет РЕЗОЛВЕР, а не твоя таблица. Одна команда, свои файлы через запятую:',
    `     cd frontend_vue && CONTRACT_REFS=roo_code/roo-context/api/${domain}.md,roo_code/plans/bugs/contract-sync-${domain}-bugs.md \\`,
    '       npx vitest run src/services/contractRefs.spec.ts',
    '   Он берёт файл ЦЕЛИКОМ и проверяет три вещи: файл существует, диапазон в границах,',
    '   утверждаемый токен лежит внутри диапазона. Строка «битых 0» по твоим файлам —',
    '   обязательное условие коммита.',
    '   ВЫВОД РЕЗОЛВЕРА ВСТАВЬ В ОТВЕТ ЦЕЛИКОМ. «Ссылок 475, провалов 0» без вывода не',
    '   принимается и считается непройденной К7: прогон 2026-09-04 так и терял домены —',
    '   автор рапортовал ноль по своей таблице, приёмщик брал файл целиком и находил остаток.',
    '   Своей таблицы ссылок не веди вовсе: два счётчика одного факта расходятся молча (Л5).',
    '10. Ссылаться на 03-api-contract.md С НОМЕРОМ СТРОКИ ЗАПРЕЩЕНО. Цитируй текстом:',
    '   «старый раздел обещал код DUPLICATE_FIELD_NAME» — и всё. Три отказа прогона',
    '   2026-09-04 из четырнадцати были именно этим (:1021 вместо :1023, :1038 вместо :1039,',
    '   573-574 вместо 575-576), а монолит всё равно удаляется задачей 38: номер строки в нём',
    '   станет мусором, даже если сегодня верен.',
    '',
...
function judgePrompt(domain, w) {
  return [
    ...
    '   Ссылки НЕ проверяй выборкой и не верь таблице автора — прогони резолвер сам:',
    `     cd frontend_vue && CONTRACT_REFS=roo_code/roo-context/api/${domain}.md,roo_code/plans/bugs/contract-sync-${domain}-bugs.md \\`,
    '       npx vitest run src/services/contractRefs.spec.ts',
    '   «битых» больше нуля — refuted = true, и перечисли их в checked строками резолвера.',
    '   Если автор заявил «провалов 0», а вывода резолвера в его ответе нет — это само по себе',
    '   основание: непроверенное заявление о ссылках и есть тот дефект, на котором прогон',
    '   2026-09-04 потерял большинство доменов.',
    '   Строки «глазами N» резолвер не считает битыми (привязка токена неоднозначна) — вот их',
    '   и посмотри глазами, это единственная часть ссылок, где нужен ты, а не команда.',
    '   Ссылка на 03-api-contract.md с номером строки — дефект независимо от того, резолвится',
    '   она или нет: монолит удаляется задачей 38, номера в нём цитировать запрещено.',
...
// skipAudit: фаза не запускается вовсе, а аудиты берутся с диска как её результат. Проверяет
// их ОДИН дешёвый агент, а не скрипт: у скрипта нет доступа к файлам (в области видимости только
// agent/phase/log/parallel/pipeline/args). Пропустить фазу можно, поверить на слово — нет:
// аудит с пустой графой отправляется в очередь и переделывается.
if (SKIP_AUDIT) {
  const check = await tryAgent(
    [
      'АВТОНОМНЫЙ РЕЖИМ. Ничего не пиши и не правь — только проверь и посчитай.',
      branchGuard(prep.branch),
      '',
      `Прогон запущен с skipAudit: аудиты доменов уже на диске, и фаза аудита пропускается.`,
      'Твоя задача — сказать, какие из них ГОДНЫ, а какие придётся переделать.',
      '',
      `Домены прогона: ${domains.join(', ')}`,
      '',
      'По каждому домену D проверь roo_code/plans/api/audit/D.md:',
      '1. файл существует;',
      "2. пустых граф ноль: grep -cE '^- [^:]+:[[:space:]]*$' — ноль означает годен;",
      "3. разделов эндпоинтов сколько: grep -c '^### '.",
      '',
      'Годен = файл есть И пустых граф ноль. Всё остальное — negoden, с указанием причины',
      'и ЧИСЛА пустых граф: домен уйдёт в очередь аудита и будет переделан.',
      '',
      'Верни ready (годные) и notReady. В notes — вывод команд по каждому домену, а не пересказ:',
    ...
    ].join('\n'),
    {
      schema: {
        type: 'object',
        properties: {
          ready: { type: 'array', items: { type: 'object', properties: { domain: { type: 'string' }, endpoints: { type: 'number' } }, required: ['domain'] } },
          notReady: { type: 'array', items: { type: 'object', properties: { domain: { type: 'string' }, reason: { type: 'string' } }, required: ['domain', 'reason'] } },
          notes: { type: 'string' },
        },
        required: ['ready', 'notes'],
      },
      label: 'skipAudit: проверка аудитов на диске',
      phase: 'Аудит',
      effort: 'low',
    },
  )
  if (!check) {
    log('skipAudit: проверяющий агент молчит — аудиты пойдут обычным путём')
  } else {
    for (const r of check.ready || []) {
      if (!domains.includes(r.domain)) continue
      audits[r.domain] = {
        domain: r.domain,
        status: 'сделано',
        endpoints: r.endpoints || 0,
        emptyFields: 0,
        notes: `взят с диска (skipAudit): roo_code/plans/api/audit/${r.domain}.md`,
      }
      results.push({ фаза: 'аудит', домен: r.domain, статус: 'взят с диска', попыток: 0, коммит: '' })
    }
    for (const n of check.notReady || []) log(`аудит ${n.domain}: skipAudit не принял — ${n.reason}`)
    log(`skipAudit: аудитов с диска ${Object.keys(audits).length} из ${domains.length}`)
  }
}

const auditQueue = SKIP_AUDIT ? domains.filter((d) => !audits[d]) : [...domains]
  ...
  if (commits.length) {
    await agent(
      [
        ...
        `Причина: ${reason}`,
        '',
        `1. СНАЧАЛА сохрани черновик — он нужен следующей попытке, чтобы правила названное, а не`,
        `   писала домен заново: mkdir -p ${DRAFT_DIR} && cp roo_code/roo-context/api/${domain}.md ${DRAFT_DIR}/${domain}.md`,
        '   Черновик лежит ВНЕ репозитория намеренно: оставленный в дереве, он попал бы в коммит',
        '   следующего домена (за 2026-09-04 такое случалось дважды) или испугал бы автора грязным',
        '   деревом. История откатывается, работа — нет.',
        `2. Откати ВСЕ коммиты этой попытки, новейший первым: ${commits.slice().reverse().join(', ')}`,
        ...
        '3. Конфликт → git revert --abort, и напиши в ответе, какие коммиты остались.',
        '4. git status --short обязан стать пустым: следующая попытка стартует с чистого дерева,',
        '   а черновик ждёт её вне репозитория.',
        `5. Допиши причину отклонения в roo_code/roo-context/verify-runs/contract-${domain}.md`,
        '',
