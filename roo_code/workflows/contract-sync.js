...
const prep = await agent(
  [
    ...
    '   Зелёная спека = скелеты сходятся с инвентарём (сумма разделов 175). Красная — останавливайся:',
    '   дальше идти нельзя, аудиты пишутся поверх скелетов.',
    '   В skeletons верни число файлов в roo_code/plans/api/audit/ кроме 00-решения-владельца.md,',
    '   в inventory — число эндпоинтов из строки «описано эндпоинтов: N из M» (это M).',
    `5. Проверь, что ${DECISIONS} существует. Нет — создай с шапкой: сюда фазы сносят строки,`,
    '   у которых источник «нигде».',
    '6. Если после шагов выше дерево стало непустым (что не должно случиться) — не коммить чужое,',
...
function auditPrompt(domain, afterCrash, attempt, lastReason, readyDeps, missingDeps, branch) {
  return [
    ...
    'Файл может оказаться УЖЕ ЗАПОЛНЕННЫМ — предыдущим прогоном или пробой. Тогда не переписывай',
    'с нуля: проверь заполненное по коду, дополни недостающее, и скажи в notes, что проверял, а не',
    'писал. Переписывание чужой верной работы дороже и теряет найденное.',
    afterCrash
    ...
    '   Ссылка не просто ставится, а ПРОВЕРЯЕТСЯ: диапазон открывают и грепают по нему',
    '   утверждаемый токен — sed -n "26,31p" <файл> | grep -c RESTRICT обязан дать >= 1.',
    '   Аудит пробы categories поставил tenant_id на миграцию :44, где на самом деле колонка id;',
    '   ошибку поймал только следующий агент, и она успела переехать в контракт дословно.',
    ...
    '   Диапазон строк ТВОЕГО домена — в таблице замеров плана, и брать надо ровно его: H1 этого',
    '   файла не совпадают с доменами. bcc и config лежат внутри H1 «Suppliers», products — внутри',
    '   «Categories», а «Логи аудита» (домен audit-feed) вклинился в середину блока Settings.',
    '   Возьмёшь блок целиком — опишешь чужой домен как свой пробел.',
...
function writePrompt(domain, audit, fixReason, afterCrash, attempt, readyNeighbours, branch) {
  ...
  return [
    ...
    '9. Каждая ссылка файл:строка проверена — не только строки «Реализация:», но и инлайновые',
    '   в абзацах, в «Правилах домена» и в «Обязанностях сервера», где ссылка и есть всё',
    '   доказательство. Проверка — открыть диапазон и грепнуть в нём утверждаемый токен;',
    '   инверсия — неверный диапазон обязан давать 0. Их сотни, резолвить скриптом, а не глазами.',
    '   Это НЕ формальность: проба categories потеряла две попытки из трёх на трёх ссылках',
    '   models.py:29-33, где утверждаемого RESTRICT нет — он на :28. К7, резолвившая только',
    '   «Реализация:», объявила при этом свип чистым.',
    '',
...
function judgePrompt(domain, w) {
  return [
    ...
    '   Плюс возьми 5 инлайновых ссылок файл:строка — из «Обязанностей сервера» и абзацев, а не',
    '   из строк «Реализация:» — и резолвни каждую: открой диапазон, грепни утверждаемый токен.',
    '   Токена в диапазоне нет — refuted = true: доказательства не существует ровно там, где на',
    '   нём всё держится.',
...
function conventionsPrompt(attempt, lastFail) {
  return [
    ...
    '',
    'СКВОЗНЫЕ ОБЯЗАННОСТИ — главная работа этой задачи, и её больше некому сделать.',
    'Аудиты писались по домену, а часть правил живёт поперёк доменов и в каждом аудите видна',
    'только куском. Собрать целиком, с ссылками на код, и там, где правила нет нигде, — строкой в',
    `${DECISIONS}, а не выдумкой:`,
    '',
    '  ПРАВА. Матрица types/config.ts:35-53 (PermissionMatrix, PermissionAction: read/edit/create/',
    '    delete) плюс GET/PUT /api/config/permissions — это домен config. Три права заказов',
    '    (seeCost, manualCost, correction, пять ролей) — GET /api/settings/order-permissions, домен',
    '    settings. Потребитель — composables/useOrderPermissions.ts, домен orders. Модель описана в',
    '    plans/orders/orders-backend-contract.md §5. Вопрос, на который обязан ответить контракт:',
    '    где сервер применяет матрицу и что отвечает при отказе (прежний текст говорил',
    "    403 { code: 'FORBIDDEN' } — проверить, так ли это в коде).",
    '  КАСТОМНЫЕ ПОЛЯ. Определения — types/config.ts:5 FieldDefinition, CRUD /api/config/fields,',
    '    привязка к категории PUT /api/categories/:id/fields. Значения — types/product.ts и',
    '    mocks/products.ts (fieldValues). ЖИЗНЕННОГО ЦИКЛА НЕТ НИГДЕ: что происходит со значениями',
    '    при удалении определения, кто валидирует тип значения — в моке config.ts об этом ни строки.',
    '    Это решение владельца, не выдумывай его.',
    '  ФИЧИ И ТАРИФЫ. Бэкенд УЖЕ содержит полную модель, а фронт о ней не знает:',
    '    billing/shared/models.py — Plan (тариф = набор фич), PlanFeature (feature_key + разрешено',
    '    ли), TenantFeatureOverride (переопределение на арендатора), FeatureDefinition («Registry of',
    '    all known feature flag keys — single source of truth», поля key, level, is_system).',
    '    Ключи сеются миграцией alembic/versions/8cf3bfa380dd_phase_12_plans_multi_role.py.',
    '    Фронт при этом держит 52 флага КОНСТАНТОЙ в config/featureFlags.ts плюс переопределения в',
    '    localStorage, и эндпоинта фич не зовёт ни одного (проверено инвентарём: ноль вызовов).',
    '    Роутов у модуля billing тоже ноль. Опиши это как есть и вынеси решения владельцу:',
    '    откуда админка берёт матрицу фич, совпадает ли словарь 52 ключей с feature_definitions,',
    '    что сервер отвечает на запрос к выключенной фиче (404, 403 или пустой ответ).',
    '  АУДИТ-ЛОГ. Девять сущностей с DELETE .../audit/:entryId, адресация по id, а не по позиции.',
    '  УВЕДОМЛЕНИЯ. Семь функций notify* в mocks/notifications.ts, зовутся из orders, warehouse,',
    '    bcc, finance. Правило «событие пишет ровно одну запись, повтор не пишет ничего» доказано',
    '    mocks/notification-triggers.spec.ts.',
    '  МУЛЬТИАРЕНДНОСТЬ. tenant_id есть в моделях всех десяти модулей бэкенда — значит фильтр',
    '    обязателен в каждом запросе, и это правило одно на все домены.',
    '  ИДЕМПОТЕНТНОСТЬ. Idempotency-Key на необратимых POST (bcc/send, bcc/log), newIdempotencyKey()',
    '    в services/api.ts.',
    '',
...
// Необратимые шаги финала разрешены, только если закрыты ВСЕ домены проекта, а не все домены
// этого прогона. Условие считается здесь, чтобы агент финала его не толковал.
const allDomainsClosed = DEFAULT_DOMAINS.every((d) => written.includes(d))
...
const final = await tryAgent(
  [
    ...
    // на половине работы.
    //
    // Сравнивать надо с ПОЛНЫМ списком доменов проекта, а не со списком этого прогона. Иначе
    // прогон на подмножестве (`args.domains: ['uploads']`) закрывает «все свои» домены — один
    // из одного — и сносит монолит, от которого не переехали шестнадцать остальных.
    `   Прогон закрыл доменов: ${written.length} из ${domains.length} в этом прогоне; всего доменов проекта ${DEFAULT_DOMAINS.length}.` +
      (allDomainsClosed
        ? ' Прогон полный и все закрыты — три шага ниже РАЗРЕШЕНЫ, если спека подтвердит 175 из 175.'
        : ' Полного покрытия нет — три шага ниже ЗАПРЕЩЕНЫ, только назови в notes, что осталось.'),
