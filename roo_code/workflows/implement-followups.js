...
function workPrompt(item, fixReason, afterCrash) {
  return [
    ...
    fixReason
      ? `\nРАБОТА НАД ОШИБКАМИ, а не чистый лист. Предыдущая попытка по этому пункту отклонена\nприёмкой. Разбор приёмщика — ниже, он проверен и ему следует верить больше, чем\nсобственному первому впечатлению:\n\n${fixReason}\n\nЧини именно названное. Остальное не переделывай: то, что приёмщик подтвердил как\nверное, трогать запрещено — переписывание принятого само по себе основание для отказа.\nЕсли разбор говорит, что код верен и не хватает только проверки — пиши проверку, а\nкод не трогай. Шаг «Воспроизведи» выполняй с поправкой на это: часть пункта уже\nсделана прошлой попыткой, и увидев её, НЕ пиши "устарело" — это не чужая давняя\nправка, а незакрытый долг по этому же пункту.\n`
      : '',
    afterCrash
...
while (queue.length) {
  ...
  let w = await agent(workPrompt(item, fixReasons[item] || null, (silentCount[item] || 0) > 0), { schema: WORK, label: `пункт ${item}`, phase: 'Работа' })
