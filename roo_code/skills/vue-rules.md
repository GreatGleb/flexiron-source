...
> **Контракт — отдельный скил.** Писать и проверять его раздел — [`api-contract.md`](api-contract.md):
> там порядок чтения кода, жёсткий формат раздела и линзы К1–К7 (инвентарь вызовов, ветка мока у
> каждого роута, каталог кодов ошибок, формы против `types/*.ts`, источник истины, обязанности
> сервера, живые ссылки). Здесь остаётся только правило write-back: тронул эндпоинт — правь его
> раздел, а `npm run verify` покажет, если раздела нет.
...
> Питфоллов сейчас **73**. Это единственное место, где их считают: ссылки из ROO.md и
...
замаскированная под её же решение. Второй такой тест жил в `warehouse.spec.ts`, вкладка
движений; там замерено то же самое (счётчик T+155 мс, первая строка T+480 мс) и правка
внесена 2026-08-30 — `plans/bugs/pagination-counter-is-not-a-data-signal.md`.
...
**Сравнивать надо ОДИНАКОВЫМ способом с обеих сторон.** `toHaveText` нормализует пробелы у
ПОЛУЧЕННОГО значения и НЕ нормализует у ожидаемого — замерено 2026-08-30 на синтетическом
узле: `innerText` вернул `alpha\nbeta`, и `toHaveText('alpha\nbeta')` не совпал. Значит
`expect(row).not.toHaveText(снятое innerText'ом)` истинно ВСЕГДА, при любом коде, — но
**только там, где innerText вставляет разделитель**, то есть у блочных детей и у ячеек
строки таблицы (`<tr>` отдаёт табы). Так было в `cutting.spec.ts`, где эталон снимался со
строки: питфолл #68, пойманный инверсией.

**Границу правила проверять, а не додумывать.** Тому же обвинению подвергли счётчик
`.pagination-info` в `warehouse.spec.ts` — и там оно оказалось ложным: `div` с тремя
СТРОЧНЫМИ `span` даёт innerText без единого переноса (`1-25 of 189`, коды
`31 2d 32 35 a0 6f 66 a0 31 38 39`), утверждение падает как положено. Дефект там был один —
не тот признак, — а не два. Способ проверки на месте: снять эталон тем же способом, что и
тест, и утвердить `toHaveText(снятое)` СРАЗУ. Не совпало — форма `not.` слепа; совпало —
...
| счёт по ВСЕМУ набору там, где действие меняет подмножество | итог сходится и при выключенном механизме, и при вывернутом наизнанку |
| правило, записанное только в комментарии рядом с ассертом | комментарий не исполняется; проверяется то, что в коде, а не то, что в тексте над ним |
...
### 🔥 #69 — Скриншот-эталон, который существует и не охраняет ничего

**Симптом:** правка меняет вид — цвет пилюли, размер бейджа, отступ — и полный прогон
проходит молча. Эталон на эту область ЕСТЬ и зелёный.

**Причина:** `toHaveScreenshot` считает пиксель разошедшимся только если цвета отличаются
больше, чем на `threshold`, а он по умолчанию **0.2**. Пока разошедшихся пикселей ноль,
`maxDiffPixelRatio` любой строгости уже ничего не решает — доля от нуля есть ноль.

Замерено 2026-08-30: смена фона, рамки и свечения у 24 пилюль страницы не покрасила ни один
эталон ни при `maxDiffPixelRatio: 0.01`, ни при `0`. Красным стало только с `threshold: 0`.

**Это хуже, чем отсутствие эталона.** Отсутствие видно, а зелёный эталон читается как «вид
не менялся» и создаёт уверенность. В этом проекте под порогом 0.2 накопилось **пять**
устаревших линий, и три из них пропустили правки, сделанные по решению владельца, — то есть
эталоны молчали ровно там, где вид менялся намеренно.

**Fix:** порог задан явно и в ОДНОМ месте — `playwright.config.ts`, `expect.toHaveScreenshot`.
Не в хелпере с опциями: часть эталонов снимается вообще без опций, и правка в хелпере их не
достанет.

```ts
expect: {
  toHaveScreenshot: { maxDiffPixelRatio: 0.01, animations: 'disabled', threshold: 0 },
}
```

**Ноль ставится по замеру, а не по идеалу** — правилом храповика, как пороги покрытия.
Порядок: прогнать эталоны при нуле ДВАЖДЫ подряд; совпал набор падений и числа пикселей —
ноль детерминирован, и каждое падение надо разобрать поимённо. Разошёлся — поднимать до
значения, при котором держится, с датой и причиной в комментарии.

**Опасение «ноль краснеет от сглаживания шрифта» проверено и не подтвердилось** — при
закреплённых шрифтах (`helpers/webFonts.ts`) рендер детерминирован. Но проверять это надо
замером, а не верить на слово ни ему, ни этому абзацу.

**Доказательство правки — КРАСНЫЙ на подкрашенном пикселе**, а не зелёный прогон: измени
цвет одного элемента и убедись, что эталон это видит. Зелёным он был и до правки.

---

### 🔥 #70 — Потолок ожидания не растягивается вместе с нагрузкой

**Симптом:** тест зелен месяцами и падает в тот день, когда рядом идёт второй прогон.
Поодиночке проходит всегда.

**Причина:** у теста потолок 90 секунд, а у `expect` — **5**, и это разные вещи. Потолок
теста задан в конфиге под худший случай; дефолтный потолок ассерта — нет. Под нагрузкой
страница отдаёт данные позже, а пять секунд остаются пятью секундами.

Замер на смене страницы под `Emulation.setCPUThrottlingRate` — время от клика до новых
строк, и отдельно то, что успевает съесть сам клик (бюджет ассерта тратится только после
него):
...
**Fix:** ожидание ПРИШЕДШИХ ДАННЫХ берёт общий бюджет, а не дефолт —
`DATA_READY_TIMEOUT` из `tests/e2e/helpers/ready.ts`, то же число, что у `waitForDataReady`.
...
Число живёт в одном экземпляре: двадцать две копии по местам разошлись бы молча.

**Чего делать НЕ надо:** поднимать глобальный `expect: { timeout }` в конфиге. Он поднимет
потолок и у `toBeVisible`, у которого пол уже есть (`openAdminPage`), и падающий прогон
станет вшестеро дольше без выигрыша.

**Цена нулевая на успехе:** `expect.poll` возвращается, как только условие выполнено.
Платит только падающий тест.

**Пишется это честно.** На rate 40 — худшей нагрузке, которой проект меряет свой потолок
теста, — пятисекундного бюджета ещё хватает с двукратным запасом. Ломается он между 40 и
100. То есть речь о ТОНКОМ запасе, а не о пробитом, и выдавать одно за другое не надо:
инверсия обязана быть на той нагрузке, где старое ожидание действительно падает.

---

### 🔥 #71 — Инверсия, которая ничего не сломала, ничего и не доказала

**Симптом:** сломал механизм, прогнал тест — зелёный. Вывод «тест слеп» напрашивается сам,
и он может быть неверным: возможно, сломано было не то место.

**Три способа промахнуться, все встречались:**

**1. Сломан не тот источник.** В `tests/e2e/mocks/*.ts` лежат моки через `page.route`, а
приложение поднимается с `VITE_USE_MOCKS=true` и отвечает **в процессе** из
`src/services/mocks/`. Сетевого запроса нет вовсе, значит `page.route` не срабатывает ни
разу. Правка такого мока не меняет ничего, и оба варианта теста остаются зелёными.

Проверено 2026-08-30: сломал `paginate()` в `tests/e2e/mocks/warehouse.ts` — и слепое, и
зрячее ожидание прошли. Та же правка в `src/services/mocks/index.ts` сразу дала нужную
картину: слепое проходит, зрячее падает.

**2. Прогон обогнал dev-сервер.** Запуск сразу после записи файла проверяет СТАРЫЙ модуль:
Vite не успел пересобрать. Ложная зелень получена дважды за одну ночь.

**3. Нагрузка не та.** См. #70: инверсия на rate 40 может пройти там, где нужен rate 100.

**Fix:** у инверсии есть **своя** приёмка. Прежде чем делать вывод — убедись, что сломанное
сломалось: хотя бы ОДИН тест обязан покраснеть. Не покраснел никто — сломано не то место,
и вывод про слепоту теста делать рано.

```
сломал → что-то покраснело? → нет → чинить инверсию, а не писать вывод
                             → да  → сравнивать варианты
```

---

### 🔥 #72 — Сведение CSS теряет унаследованные свойства молча

**Симптом:** четыре копии одного класса свели в одну, обещав «на экране ничего не
изменится», — и что-нибудь всё-таки изменилось: регистр букв, начертание, межбуквенный
интервал.

**Причина:** победивший вариант мог не объявлять свойство вовсе, а получать его
**наследованием** — от соседнего объявления в том же файле, от родительского селектора, от
`*`. После сведения наследовать стало неоткуда, и свойство пропало.

Реальный случай: `text-transform: uppercase` доставался выбранному варианту от соседнего
объявления. В сведённом виде его никто не написал, и пилюли молча потеряли заглавные.

Опасны именно **наследуемые** свойства — `text-transform`, `font-*`, `letter-spacing`,
`line-height`, `color`, `text-align`, `white-space`, `visibility`. Ненаследуемые
(`padding`, `border`, `background`) так исчезнуть не могут.

**Fix:** перед сведением снять **computed style** победителя, а не читать исходники. Всё,
что в нём есть и относится к предмету, пишется в сведённом правиле ЯВНО — включая то, что в
исходнике не было объявлено.

Проверка после — тем же computed style на тех же маршрутах, а не «выглядит так же».

---

### 🔥 #73 — Порядок впрыска постраничного CSS в SPA зависит от маршрута

**Симптом:** одна и та же страница выглядит по-разному в зависимости от того, как на неё
попали. Прямой загрузкой пилюля 59×22, приходом навигацией с другого раздела — 71×25, и
таблица от второго становится настолько шире, что колонка действий обрезается.

**Причина:** два непрефиксных объявления одного класса в разных постранично загружаемых
файлах. Вес одинаковый, значит побеждает тот, что впрыснут последним, — а порядок впрыска
задаёт маршрут пользователя, не сборка.

**Fix:** побеждать должна **специфичность**, а не порядок. Постраничное объявление
префиксуется страницей (`.page-suppliers .status-pill` — вес (0,2,0) против (0,1,0)), либо
класс сводится в один общий файл, если различие не задумано.

**Как это искать — два правила, оба выведены из промахов:**

1. **Греп по `src/styles/**/*.css` неполон.** Объявление может лежать в `<style>` внутри
   `.vue`, и там **отсутствие `scoped` делает правило глобальным**. Искать надо по обоим
   местам;
2. **Прямыми загрузками страниц течь не воспроизводится.** Она проявляется только на
   некоторых путях — в разобранном случае на обратном (поставщики → склад → назад), а на
   прямом нет. Проверять — обходом по приложению **в обе стороны**, сравнивая computed
   style, а не открытием каждой страницы по отдельности.

**Регрессия этого не ловит сама по себе:** видимое изменение вида проходит полный прогон
молча, если у модуля нет скриншот-эталонов, — а если есть, но с дефолтным порогом, молчит и
он (#69).
