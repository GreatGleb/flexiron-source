...
**Не найден нигде — баг. Найден в ДВУХ местах непрефиксно — тоже баг (#73).** Два одноклассовых объявления в разных постранично загружаемых файлах имеют одинаковый вес, значит побеждает впрыснутый последним, а порядок впрыска в SPA задаёт маршрут пользователя. Одна и та же страница показывает разный вид в зависимости от того, как на неё попали. Проверка: грепнуть класс по `src/styles/**/*.css` **и** по `<style>` внутри `.vue` (без `scoped` правило глобально); нашлось больше одного непрефиксного объявления — префиксовать постраничное страницей либо свести в общий файл.

Проверять это прямыми загрузками страниц НЕЛЬЗЯ: течь проявляется только на некоторых путях — обходом в обе стороны, сравнивая computed style.
