...
**Слайс появился — метка в контракте истекла.** Если раздел этого эндпоинта в
`roo_code/roo-context/api/<домен>.md` стоял с `**Статус:** спроектировано`, метка снимается той же
задачей, а форма ответа сверяется со схемами слайса: с этого момента источник истины — бэкенд, а не
мок — линза К5 в [`api-contract.md`](api-contract.md). Спека `contract-conformance.spec.ts`
покраснеет сама, если метка осталась.
