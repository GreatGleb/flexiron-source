...
Если пользователь упоминает страницу, баги, этап работы, раздел, или продолжение задачи — **НЕМЕДЛЕННО**:

1. `Read .claude/skills/orchestrate.md`
2. Определить Plan ID и текущий этап по файлам
3. Объявить этап пользователю и продолжить

**НЕ ждать команды. НЕ использовать `Skill()`. НЕ отвечать "жду список" без запуска оркестратора.**

Примеры триггеров: "остановился на этапе багов", "продолжи страницу", "создай страницу", "пришлю список багов", "возобнови", любое упоминание раздела/страницы в контексте разработки.
