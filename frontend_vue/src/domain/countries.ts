 ...
 * Литва — локальный, любая другая — экспорт. Веток ровно две.
 *
 * Незаполненная страна — это «любая другая», и третьей ветки «предлагать
 * нечего» здесь нет намеренно. Она выглядит осторожной ровно до того момента,
 * пока не посмотреть, что стоит в форме заказа вместо предложения: у
 * `documentType` есть значение по умолчанию — `'local'`. Значит промолчать —
 * не воздержаться, а оставить локальный комплект, а с ним 21 % НДС и накладные
 * LT в заказе для немецкой фирмы, у которой страна не заполнена (форма клиента
 * страну не требует). Из двух молчаливых исходов выбран тот, который не
 * выставляет клиенту чужой налог.
 */
