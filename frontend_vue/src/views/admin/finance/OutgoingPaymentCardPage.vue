...
<template>
  ...
  <template v-else-if="payment">
    ...
    <div class="entity-card-grid">
      ...
      <!-- CENTER: Amount, dates, supplier invoice ref -->
      <div class="entity-col-center">
        <GlassPanel :loading="loading" :skeleton-rows="4" data-test="payment-card-center-panel">
          <template v-if="payment">
            ...
            <div class="input-group">
              <label class="field-label">{{ t('financePayment.supplier_invoice_ref') }}</label>
              <input
                class="glass-input"
                :value="payment.supplierInvoiceRef ?? '—'"
                type="text"
                readonly
                data-test="payment-field-invoice-ref"
              />
            </div>
          </template>
        </GlassPanel>
