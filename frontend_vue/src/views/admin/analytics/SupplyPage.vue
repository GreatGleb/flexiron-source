...
<template>
  ...
  <template v-else-if="data">
    <div class="kpi-row" data-test="supply-kpi-row">
      <div v-for="kpi in data.kpis" :key="kpi.key" class="kpi-card" data-test="supply-kpi-card">
    ...
    <div class="charts-row" data-test="supply-charts-row">
      <div class="glass-panel" data-test="supply-suppliers">
        ...
        <table class="data-table" data-test="supply-suppliers-table">
          ...
          <tbody>
            <tr
              v-for="(supplier, idx) in data.suppliers"
              :key="idx"
              data-test="supply-supplier-row"
            >
              <td>{{ tf(supplier.name) }}</td>
              <td class="hid-320">{{ supplier.deliveries }}</td>
              <td>{{ supplier.ontime }}%</td>
              <td>
                <span :class="['status-pill', 'pill-' + supplier.statusType]">{{
                  tf(supplier.status)
                }}</span>
              </td>
            </tr>
