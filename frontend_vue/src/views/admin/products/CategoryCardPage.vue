...
const parentOptions = computed(() => [
  ...
  ...allCategories.value
    .filter((c) => c.id !== id)
    .map((c) => ({ value: c.id, label: tf(c.name) })),
])
    ...
    <div class="page-category-card" data-test="page-category-card">
      <div class="category-card-header" data-test="category-card-header">
        ...
        <div class="category-card-header-row">
          <h1 class="page-title">
            {{ category?.name ? tf(category.name) : t('categories.title') }}
          </h1>
      ...
      <div class="category-panels-grid">
            ...
            <p class="inherited-label">
              ...
              {{
                allCategories.find((c) => c.id === category?.parentId)?.name
                  ? tf(allCategories.find((c) => c.id === category?.parentId)!.name)
                  : ''
              }}
            </p>
            <table class="data-table">
              <tbody>
                <tr v-for="field in category.inheritedFields" :key="field.id">
                  <td>{{ tf(field.name) }}</td>
                  <td>
                    <span class="tag tag-sm">{{ t(`categories.type_${field.type}`) }}</span>
                  </td>
                  <td>
        ...
        >
          ...
          <table
            v-if="linkedSuppliers.length"
            class="data-table"
            data-test="category-suppliers-table"
          >
            <thead>
              <tr>
      ...
      <AppModal
        ...
        :title="
          editingField
            ? t('categories.modal_field_title_edit')
            : t('categories.modal_field_title_add')
        "
        size="medium"
