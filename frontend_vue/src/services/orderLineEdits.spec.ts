...
describe('every refusal reaches the admin as a sentence', () => {
  ...
  /**
   * Все случаи выше строят ошибку так, как её бросает мок, — кодом в тексте. Настоящий
   * сервер так не делает, и до 2026-09-11 на его форме ни один из 46 кодов не находился:
   * двадцать пять вызовов в карточке и списке отдавали общий «не удалось сохранить».
   */
  it('reads the code the real server sends — the field, not its sentence', () => {
    const fromServer = new ApiRequestError({
      status: 409,
      message: 'The line is already on an invoice and cannot be changed',
      code: 'LINE_ON_INVOICE',
    })
    expect(lineEditErrorKey(fromServer)).toBe('orders.error_line_on_invoice')
  })

  it('does not let the server sentence stand in for the code', () => {
    const misleading = new ApiRequestError({
      status: 409,
      message: 'ZERO_QUANTITY appears in this sentence',
      code: 'LINE_FULLY_SHIPPED',
    })
    expect(lineEditErrorKey(misleading)).toBe('orders.error_line_fully_shipped')
  })
})
