...
describe('ссылки документов контракта', () => {
  it('отчёт по каждому документу; экстрактор обязан что-то находить', () => {
    ...
    //
    // `api/README.md` из пола исключён, и это не поблажка: он не проза, а сгенерированная карта
    // «метод путь → файл», ссылок `файл:строка` в нём нет по своей природе — их там неоткуда
    // взять. Пол на нём проверял бы не экстрактор, а факт генерации карты, и краснел бы вечно.
    // Ссылки самой карты проверяет свой сторож — `contractReadme.spec.ts`.
    const filled = reports.filter(
      (r) =>
        /\/(api|audit)\//.test(r.file) &&
        !r.file.endsWith('/README.md') &&
        readFileSync(resolve(ROOT, r.file), 'utf8').length > 8000,
    )
