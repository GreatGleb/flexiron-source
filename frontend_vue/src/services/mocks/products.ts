...
// `avgCostPrice` and `avgSalePrice` are `null` on every seeded product on purpose.
...
// The cost average was a weighted average of the RAW seed prices, taken from the
// same array the warehouse repriced a moment later — so every stocked product
// quoted a figure none of its own batches supported. The sale average was the
// catalogue price copied across under the label "average sale price", which
// averages nothing and shows 0,00 for a product that has no price at all.
//
// A derived number that is stored is a number that can be wrong. These two are
// asked for at the moment they are read, from whoever owns the answer:
// the warehouse knows what is on the shelf, the orders module knows what shipped.

/** What the warehouse answers about one product's shelf. */
export interface StockedBatchView {
  quantityRemaining: number
  /** Warehouse cost in the base currency; `null` when the batch has none. */
  unitPrice: number | null
  currency: string
}

/** What the orders module answers about one product's sales. */
export interface ShippedSalesView {
  /** Quantity actually shipped, over orders that were not cancelled. */
  quantity: number
  /** Net of what shipped — after every discount and correction. */
  net: number
}

let batchLookup: ((productId: string) => StockedBatchView[]) | null = null
let salesLookup: ((productId: string) => ShippedSalesView | null) | null = null

/**
 * Registered by the warehouse module — see the note at its call site.
 * Products cannot import the warehouse: the warehouse imports products, and a
 * cycle here would let import order decide whether the demo store exists.
 */
export function registerProductBatchLookup(
  lookup: (productId: string) => StockedBatchView[],
): void {
  batchLookup = lookup
}

/**
 * Registered by the orders module, the same way, and for the same reason.
 *
 * Until it is registered, `avgSalePrice` is `null` everywhere — no answer, rather
 * than a wrong one. The registration itself belongs in `mocks/orders.ts`, next to
 * `registerClientOrderLookup`; see the report for the exact shape.
 */
export function registerProductSalesLookup(
  lookup: (productId: string) => ShippedSalesView | null,
): void {
  salesLookup = lookup
}

/**
 * Σ(unitPrice × quantityRemaining) ÷ Σ quantityRemaining over the batches that
 * still hold stock, in the base currency (§7.2).
 *
 * Nothing on the shelf → no average. A batch nobody priced is left out of both
 * sums: an unknown cost is not a cheap one, and averaging it in as zero would
 * quietly write the shelf down.
 */
export function avgCostPriceFor(productId: string): number | null {
  const batches = (batchLookup?.(productId) ?? []).filter(
    (b) => b.quantityRemaining > 0 && b.unitPrice != null,
  )
  const quantity = batches.reduce((sum, b) => sum + b.quantityRemaining, 0)
  if (quantity <= 0) return null
  const value = batches.reduce((sum, b) => sum + b.quantityRemaining * (b.unitPrice ?? 0), 0)
  return round2(value / quantity)
}

/**
 * Σ(net of what shipped) ÷ Σ(shipped quantity) over orders that were not
 * cancelled (§7.2).
 *
 * Never shipped → no average. Zero would be a claim that the goods went out for
 * nothing; the card shows a dash instead.
 */
export function avgSalePriceFor(productId: string): number | null {
  const sold = salesLookup?.(productId) ?? null
  if (!sold || sold.quantity <= 0) return null
  return round2(sold.net / sold.quantity)
}

/**
 * Hangs the two averages off a product as derived properties.
 *
 * They read as ordinary fields — `product.avgCostPrice` — and are enumerable, so
 * they survive a spread onto the wire. Writing to them does nothing: a number
 * derived from the shelf and from the orders is not something a client gets to
 * set, and the seeded literals in this file are inert for the same reason.
 */
function defineDerivedAverages(product: Product): void {
  Object.defineProperty(product, 'avgCostPrice', {
    get: () => avgCostPriceFor(product.id),
    set: () => {},
    enumerable: true,
    configurable: true,
  })
  Object.defineProperty(product, 'avgSalePrice', {
    get: () => avgSalePriceFor(product.id),
    set: () => {},
    enumerable: true,
    configurable: true,
  })
...
function toListItem(p: Product): ProductListItem {
  return {
    ...
    // Both averages travel, or the list quietly answers a different question than
    // the card does about the same product.
...
export async function mockCreateProduct(
  ...
  const product: Product = {
    ...
    // has nothing on the shelf and nothing shipped, so both answer `null`.
    avgCostPrice: null,
    avgSalePrice: null,
...
export async function mockPatchProduct(
  ...
  const patched: Product = {
    ...
    // properties are hung on it below. Neither average is a stored field, so
    // neither can be patched — the shelf and the shipped orders decide them.
    avgCostPrice: null,
    avgSalePrice: null,
