...
describe("the client's issued invoices", () => {
  /**
   * The client card asks one question — "what has this client been billed" — and
   * the answer has to survive a correction. A withdrawn document is still a row
   * (it was issued, and the accountant will ask about it), but it is no longer
   * money anybody owes, and a column summed without that distinction bills the
   * client twice for a delivery that came back.
   */
  it('lists a document per order, with the money that went against it', () => {
    ...
    const rows = mockGetClientInvoices(client.id).filter((r) => r.orderId === created.id)
  ...
  it('an advance invoice is on the list too, and belongs to the client who was billed', () => {
    ...
    const advance = mockCreateInvoice(created.id, { kind: 'advance', amountNet: 500 })

    const rows = mockGetClientInvoices(client.id).filter((r) => r.orderId === created.id)
    expect(rows).toHaveLength(1)
    expect(rows[0]!.kind).toBe('advance')
    expect(rows[0]!.amountGrossCurrent).toBe(advance.amountGross)
    // And it is not on anybody else's list.
    expect(mockGetClientInvoices(other.id).some((r) => r.orderId === created.id)).toBe(false)
  })
})
