...
export default tseslint.config(
...

  {
    ...
    rules: {
      /*
       * 186 срабатываний, из них 170 ложных: правило не считает утверждением
       * `expect.soft()`, а это основной стиль набора — мягкие проверки дают
       * все расхождения разом, а не первое. Оставшиеся 16 тестов утверждений
       * действительно не содержат; они записаны находкой, а не заметены сюда.
       */
      'sonarjs/assertions-in-tests': 'off',
      /*
       * 10 срабатываний, и все настоящие: `waitForTimeout` — зависимость от
       * времени, которого тест не контролирует, то есть будущий флейк. Замена
       * каждого на честное ожидание — отдельная задача, а не побочный эффект
       * включения линта: наугад заменённое ожидание превращает флейк в падение.
       */
